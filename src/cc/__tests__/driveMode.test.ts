...
describe("arch/session-resumption.md names every drive-mode wire string", () => {
  ...
  it("is not vacuous — the doc loaded, is the right one, and is not truncated", () => {
    // ⚠️ A HEAD ANCHOR CANNOT SEE TRUNCATION — truncation eats the TAIL. Pin one section
    // near the top and one near the bottom, so a file cut in half satisfies the first and
    // fails the second. This replaces a bare `length > 5_000` byte threshold, which was the
    // wrong instrument: the number was underived (this doc is ~29 KB, so 5 KB was ~6× slack
    // and would sail past a gut-to-20%), and an unexplained numeric threshold is the shape a
    // future reader LOWERS on first failure rather than investigates.
    expect(archDoc).toContain("### The drive-mode signal");
    expect(
      archDoc,
      "arch/session-resumption.md is TRUNCATED — its last section is missing (a head-only " +
        "check would have passed this). Do not relax the assertions below to compensate.",
    ).toContain("### Verification method banked here");
  });

  // ⚠️ ANCHORED TO THE TABLE ROW, NOT THE BARE WORD — and the difference is not theoretical.
  // `stepping`, `orchestrated` and `fsd` each occur exactly once in this doc, so a bare
  // `toContain(mode)` happens to be decisive for them. **`autopilot` occurs 4×**, three of
  // them incidental prose elsewhere in the file — so that one arm would have SURVIVED
  // deletion of the very vocabulary table it exists to protect. Because the arms are
  // generated by `it.each`, that asymmetry was invisible: three arms would die under
  // mutation and one would not, and nothing in the output says which.
  //
  // ⚠️ The row prefix (mode + its numbered label) was `grep -c`'d before being relied on:
  // each occurs exactly ONCE. The bare `| \`autopilot\` |` cell is NOT unique either — it
  // also appears in the picker-cell state table further down, which is why the anchor
  // carries the mode NUMBER rather than just the cell.
  const VOCABULARY_ROW: Record<DriveMode, string> = {
    stepping: "| `stepping` | 1 — Stepping |",
    orchestrated: "| `orchestrated` | 2 — Orchestrated |",
    autopilot: "| `autopilot` | 3 — Autopilot |",
    fsd: "| `fsd` | 4 — FSD |",
  };

  it.each(DRIVE_MODES)(
    "names the `%s` wire string in its vocabulary row",
    (mode) => {
      expect(
        archDoc,
        `arch/session-resumption.md no longer carries the vocabulary row for the drive mode ` +
          `"${mode}". It is the authority on drive mode, so a reader reconstructing the ` +
          `vocabulary from it must get all four — and a wrong guess (\`full-autopilot\`, ` +
          `\`step-by-step\`) fails serde on read and takes the whole project list down. ` +
          `Restore the row; do not weaken this to a bare-word search, which passes on ` +
          `incidental prose.`,
      ).toContain(VOCABULARY_ROW[mode]);
    },
  );
});
