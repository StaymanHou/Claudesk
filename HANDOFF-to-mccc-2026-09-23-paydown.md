...
3. **Deleting an ES-module export and migrating its consumers must land in the SAME phase.** (`SURFACE-2026-08-25-A-DELETED-EXPORT-BREAKS-THE-APP-AT-RUNTIME-NOT-JUST-TSC`, upstream half) A missing ESM export is a **runtime** module-resolution failure, not only a `tsc` error: `SyntaxError: Importing binding name 'inertAfter' is not found` blanked the whole app while `verify:auto` and `tsc` were both green, and a verify cycle was voided. **Ask:** in `feature-plan`, "does this phase remove or rename an export an un-migrated consumer still imports?" is a **hard blocker** on splitting across phases. After any phase that deletes something, a **boot smoke test** (load the app, assert `#root` has children) runs before any live observation is accepted. *(Claudesk built its local half in paydown WP3, 2026-09-23. ⚠️ **One finding changes how this rule should be worded upstream:** a boot smoke test run under a unit-test runner cannot see a missing export. Vitest's module runner reads the missing binding as `undefined` and does not throw, so a jsdom boot passes on exactly this defect. The check only works in a LIVE webview ("load the app, assert `#root` has children" is right there) or through a real ESM linker, such as a `write: false` production build, which Claudesk now runs as `pnpm check:link`. Word the rule so an agent cannot satisfy it with a runner-based import.)*
