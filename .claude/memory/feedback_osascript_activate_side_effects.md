...
**Why:** On 2026-06-16 during the WP3 Sublime CLI probe, repeated `osascript activate` calls on Sublime Text — used to measure focus theft mechanically — yanked the user's live ST windows across multiple Desktops onto the active one. The measurement intent was harmless ("which app is frontmost after this command?"); the _instrument_ was destructive. Recovery required manual Mission-Control window-dragging. The probe data was good but the user-state cost was not.
