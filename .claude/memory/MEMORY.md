...
- [git-checkout-no-ops-on-untracked-file.md](git-checkout-no-ops-on-untracked-file.md) — `git checkout -- <path>` on an UNTRACKED file does nothing and exits 0; it left a mutation probe in place. Capture `shasum` before mutating and verify after — for an untracked file `git diff`/`git checkout` are INAPPLICABLE, not weak.
...
- [behavioral-test-can-still-be-an-equivalent-mutant.md](behavioral-test-can-still-be-an-equivalent-mutant.md) — A LANDED mutation can survive a genuinely behavioral test when the two paths give identical observables on its input (determinism is the usual cause). Ask what input makes the versions differ — and run mutants INDIVIDUALLY; 2 of 4 survived invisibly inside a 98-green run.
...
- [handoff-detail-can-be-wrong-while-its-conclusion-is-right.md](handoff-detail-can-be-wrong-while-its-conclusion-is-right.md) — A session handoff's CONCLUSION can be right while a supporting detail is wrong; verify shipped-state claims against `git tag`/`git log` before writing a release note from it. An accurate adjacent number (commits-ahead) is what lends the wrong claim credibility.
