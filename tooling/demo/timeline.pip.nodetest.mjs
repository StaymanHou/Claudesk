...
test("pip timeline: opens in the pip region", () => {
  assert.equal(
    regionOf(T.keyframes[0]),
    "pip",
    "first keyframe must be the pip composition",
  );
});

test("pip timeline: has a corner pipPos (the always-on-top panel is pinned)", () => {
  assert.ok(
    T.pipPos && typeof T.pipPos === "object",
    "expected a pipPos object",
  );
  assert.ok(
    Object.keys(T.pipPos).length >= 1,
    "pipPos must pin at least one edge",
  );
});

test("pip backdrop: is ACTIVE — progressive messages + a typing draft that sends", () => {
  assert.ok(
    T.backdropLive && typeof T.backdropLive === "object",
    "expected backdropLive",
  );
  const msgs = T.backdropLive.messages;
  assert.ok(
    Array.isArray(msgs) && msgs.length >= 2,
    "expected >=2 backdrop messages",
  );
  assert.ok(
    msgs.some((m) => typeof m.at === "number" && m.at > 0),
    "expected a message that pops in over time",
  );
  const d = (T.backdropLive.typing || []).find(
    (x) => typeof x.sendAt === "number",
  );
  assert.ok(d, "expected a typing draft with a sendAt");
  const landed = msgs.find((m) => m.text === d.text);
  assert.ok(
    landed && landed.at >= d.sendAt - 0.01,
    "the sent draft must land as a message at/after sendAt",
  );
});
...
test("pip phase: exactly 2 workspace mirror cells (round-2: not 4)", () => {
  ...
  for (const [i, k] of beats.entries()) {
    assert.ok(
      Array.isArray(k.pip) && k.pip.length === 2,
      `pip beat ${i}: expected exactly 2 rows`,
    );
    for (const row of k.pip) {
      assert.equal(typeof row.name, "string", `pip beat ${i}: row.name string`);
      assert.ok(
        STATUS_VOCAB.has(row.status),
        `pip beat ${i}: bad status "${row.status}"`,
      );
      assert.ok(
        row.mirror &&
          Array.isArray(row.mirror.lines) &&
          row.mirror.lines.length > 0,
...
test("pip phase: a mirror flips to AWAITING (the ping — CC needs you now)", () => {
  const hasAwaiting = pipBeats().some((k) =>
    k.pip.some((r) => r.status === "awaiting"),
  );
  assert.ok(
    hasAwaiting,
    "at least one pip beat must show a mirror in 'awaiting'",
  );
});
...
test("⌘+Tab: a keycap shows the switch gesture", () => {
  assert.ok(
    ...
    "expected a keycap event",
  );
  const cmdTab = T.keycaps.find(
    (e) =>
      (e.keys || []).includes("⌘") &&
      (e.keys || []).some((k) => k === "⇥" || k.toLowerCase?.() === "tab"),
  );
...
test("FIDELITY: the loop SWITCHES region pip → filmstrip (the real Claudesk window, not a PiP focus ring)", () => {
  ...
  assert.ok(
    firstFilm > 0,
    "expected a later keyframe to switch to the filmstrip (real-window) composition",
  );
  // the switch must happen after the ping (pip-awaiting) — you ⌘+Tab in RESPONSE to the ping
  const pingT = pipBeats().find((k) =>
    k.pip.some((r) => r.status === "awaiting"),
  )?.t;
  assert.ok(
    typeof pingT === "number" && T.keyframes[firstFilm].t > pingT,
    "the region switch must come after the ping",
  );
});
...
test("FIDELITY: on the filmstrip stage, tax-cruncher is promoted (active) showing an AskUserQuestion", () => {
  ...
  assert.ok(
    beats.length >= 2,
    "expected >=2 filmstrip-region beats (answer + resume)",
  );
  for (const [i, k] of beats.entries()) {
    assert.ok(
      Array.isArray(k.tiles) && k.tiles.length >= 1,
      `film beat ${i}: needs tiles`,
    );
    assert.ok(
      Number.isInteger(k.active) && k.active >= 0 && k.active < k.tiles.length,
      `film beat ${i}: active in range`,
    );
    assert.ok(
      k.stage && Array.isArray(k.stage.lines) && k.stage.lines.length > 0,
      `film beat ${i}: center stage lines`,
    );
  }
  const askqBeat = beats.find((k) =>
  ...
  assert.ok(
    askqBeat,
    "a filmstrip beat must render the AskUserQuestion on center stage",
  );
  const opts = askqBeat.stage.lines.filter((l) =>
    (l.cls || "").startsWith("askq-opt"),
  );
...
test("FIDELITY: the answer happens (an option becomes selected) and CC then resumes running", () => {
  const beats = filmBeats();
  const selected = beats.some((k) =>
    (k.stage.lines || []).some(
      (l) =>
        (l.cls || "").includes("askq-opt") && (l.cls || "").includes("sel"),
    ),
  );
  assert.ok(
    selected,
    "a filmstrip beat must show a selected AskUserQuestion option (the answer)",
  );
  const resumes = beats.some(
    (k) => k.stage.busy && typeof k.stage.busy.startT === "number",
  );
  assert.ok(
    resumes,
    "after answering, the center-stage CC session must resume (a busy spec)",
  );
  // and the promoted tile returns to running
  const tileRuns = beats.some(
    (k) => k.tiles[k.active] && k.tiles[k.active].status === "running",
  );
  assert.ok(
    tileRuns,
    "the promoted tile must return to 'running' after the answer",
  );
});
...
test("mouse: the cursor works in Slack (a reaction click), NOT to switch/answer", () => {
  assert.ok(
    ...
    "expected a cursor track",
  );
  const clicks = T.cursor.filter((w) => w.click === true);
  assert.ok(
    clicks.length >= 1,
    "expected a click (the operator reacting in Slack)",
  );
  ...
  for (const c of clicks) {
    assert.ok(
      c.x < 700 && c.y < 300,
      `cursor click (${c.x},${c.y}) must be in the Slack area, not the PiP/stage`,
    );
    assert.ok(
      c.t < 4.0,
      `cursor click at t=${c.t} must happen during the monitoring beats, before the ping/switch`,
    );
  }
});
...
test("mouse: a 👍 reaction is added on a backdrop message (the Slack busywork)", () => {
  ...
  assert.ok(
    Array.isArray(rx) && rx.length >= 1,
    "expected a reaction in backdropLive",
  );
  for (const r of rx) {
    ...
    assert.equal(
      typeof r.msgIndex,
      "number",
      "reaction needs a target msgIndex",
    );
    assert.ok(
      typeof r.emoji === "string" && r.emoji.length > 0,
      "reaction needs an emoji",
    );
    assert.ok(
      r.msgIndex >= 0 && r.msgIndex < T.backdropLive.messages.length,
      "msgIndex in range",
    );
  }
  ...
  assert.ok(
    rx.some((r) => clickTs.some((ct) => Math.abs(ct - r.at) < 0.4)),
    "a reaction should coincide with a cursor click",
  );
});
...
test("keyboard: BOTH a ⌘+⇥ switch AND a 1+⏎ answer keycap are shown", () => {
  ...
  const answer = keys.find(
    (e) =>
      (e.keys || []).includes("1") && (e.keys || []).some((k) => k === "⏎"),
  );
  assert.ok(cmdTab, "expected the ⌘+⇥ switch keycap");
  assert.ok(
    answer,
    "expected the 1+⏎ answer keycap (answering in the real workspace)",
  );
  // the answer keycap must fire during/after the region switch (it's answered in Claudesk)
  const firstFilm = T.keyframes.find(
    (k) => (k.region || T.region) === "filmstrip",
  );
  assert.ok(
    answer.from >= firstFilm.t - 0.5,
    "the 1+⏎ answer must come at/after the Claudesk-window switch",
  );
});
