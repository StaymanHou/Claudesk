...
if (!file) {
  console.error(
    "usage: node parse-powermetrics.mjs <powermetrics-output.txt> [--warmup N]",
  );
  process.exit(1);
}
...
const PROC_PATTERNS = [
  {
    key: "WebContent",
    re: /WebKit\.WebContent|com\.apple\.WebKit\.WebContent/,
  },
...
if (samples.length === 0) {
  console.error(
    "No WebKit/Claudesk process rows found. Check the powermetrics output format and PROC_PATTERNS.",
  );
...
console.log(
  `samples: ${samples.length} total, ${kept.length} after ${warmup} warm-up discarded`,
);
console.log(
  `webview (WebContent+GPU) %CPU:  median=${w.median.toFixed(1)}  p95=${w.p95.toFixed(1)}  max=${w.max.toFixed(1)}`,
);
console.log(
  `main (Claudesk) %CPU:           median=${m.median.toFixed(1)}  p95=${m.p95.toFixed(1)}  max=${m.max.toFixed(1)}`,
);
console.log("");
console.log(
  "NOTE: confirm the column heuristic against the raw file — powermetrics layout varies by macOS version.",
);
console.log(
  "If numbers look wrong, inspect the raw table header and adjust PROC_PATTERNS / column pick.",
);
