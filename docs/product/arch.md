...
stage: arch
state: complete
updated: 2026-06-27  # M5 (PiP) finalize resync — §B.3 rewritten as-built (tauri-nspanel API, NonactivatingPanel not no_activate, 4 layouts + tri-state PipMode + content-driven resize, roster divergence, run-on-main-thread rule, MCP-bridge verify-self); §B.2 + decisions corrected (PiP ships before menu-bar, unconditional)
