...
- **Ships AFTER PiP** (resequenced 2026-06-22). The earlier "menu-bar before PiP, dogfood-gate PiP" plan is **superseded**: PiP (M5) shipped first and unconditionally; the menu-bar is now M6's neighborhood (still forward-look, not built). The right-click "Toggle PiP" entry remains an M6/M7 menu-bar surface — the in-Claudesk PiP icon + View-menu radio (B.3) are the as-built PiP controls.
