...
- ~~`tauri-plugin-fs-watch` / `notify` — debounced file-watcher for `workflow/.session.md`.~~ **DROPPED at M3 (2026-06-22):** `.session.md` is a manual pause bookmark (written by `/session-pause`, deleted by `/session-resume`), not a live workflow-state stream — watching it yields a near-constant, trivially-derivable signal. M3 ships the CC-state indicator from the hook channel alone. A future milestone may watch the live workflow doc hierarchy (`roadmap → wbs → wip(s) → backlog`) instead — `SURFACE-2026-06-22-WP5-DROPPED-WATCH-WORKFLOW-DOC-HIERARCHY`. (`notify`/`tauri-plugin-fs-watch` is still the watcher tech when that lands.)
...
- **CC hook registration.** *(As-built M3 WP2/WP3, 2026-06-22.)* On launch, Claudesk installs entries in `~/.claude/settings.json`'s `hooks` block for `UserPromptSubmit` (→ "running"), `Stop` (→ "idle"), and `Notification` (→ "awaiting-input"), additively/idempotently/reversibly (preserves claude-time, notify-telegram, etc.). The hook is a tiny **Perl** script (`resources/claudesk-hook.pl`, `/usr/bin/perl` — measured ~15 ms/call; the WP1 probe chose Perl over POSIX sh) that reads the event JSON on stdin and writes one JSON line — `{ hook_event_name, session_id, cwd, timestamp, prompt?, message? }` — to Claudesk's Unix socket at the stable path `<app-data>/hook.sock` (on macOS `~/Library/Application Support/com.claudesk.app/hook.sock` — the bundle identifier, **not** `Claudesk/`; `hook_socket::commands::hook_socket_path` is the single source of truth for this path).
