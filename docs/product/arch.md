...
- **Sublime *Text* pop removed at M2 — gated, last; Sublime *Merge* KEPT permanently.** The `sublime` module + `sublime_open` command + the Sublime-Text pop chord (⌘⇧E → reassigned to ⌘⇧O in WP5) + the "Open in Sublime" toolbar button are **deleted** once the in-app editor proves daily-use parity (WP8). This is the *last* M2 build step, after dogfooding the editor — if a relied-on Sublime gesture turns out hard in CM6, that gate surfaces it. The ⌘⇧E chord is freed for the Editor panel (WP5). **Sublime Merge is the deliberate exception — it is NOT removed.** A permanent `smerge_open` command + "Open in Sublime Merge" toolbar button (added in WP5) persist past WP8: the in-app DiffPanel covers inline *viewing* of the working diff, but staging / blame / history / inspecting a file at a past revision stay in Sublime Merge. *(Revised 2026-06-20, WP5 spec — the prior wording implied the whole Sublime pop, Text and Merge alike, was removed at parity.)*
