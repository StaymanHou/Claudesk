...
state: complete
milestone: 10
updated: 2026-07-17  # M10 WP3 (brew detect-and-defer gate) SHIPPED — install_source() /Caskroom/ component-match + updater_check/updater_apply short-circuit to `brew upgrade` for brew installs (direct-download self-updates); folded 2 WP2 doc-drift findings; 15 updater tests; live verify-self DirectDownload non-regression; real-brew detection DEFERRED→WP6. 2/6 WPs done (WP2, WP3). Remaining: WP4 (UX) ∥ WP5 (/release) → WP6 (exit). Local/uncommitted (batched M10 tree, commit-only-when-asked).
---
...
### WP3: Install-source detection — the brew detect-and-defer gate ✅ SHIPPED 2026-07-17 (uncommitted — batched on the unpushed M9+M10 tree, HEAD `2592b2d`)
