...
## WP5 — Comment/doc-drift sweep (incl. D2 editor-shell docs)  `[impact: Low · effort: Small · risk: XS]`  `← comment-only bulk`  ✅ DONE 2026-06-30
**Outcome:** Comment/doc-only sweep across 11 backend + 6 frontend + 6 `tooling/demo/` findings. **Pattern held: ~4 of the listed findings were stale premises already-corrected-in-place** (the recurring class this sweep keeps hitting), confirmed + marked rather than re-fixed. One actual code deletion (dead PiP focus-flag/CSS). Behavior-preserving. Gate: `tsc` + eslint + `vite build` + vitest **784** + 8/8 demo nodetests; `cargo test --lib` **307** + clippy `-D warnings` + rustdoc `broken_intra_doc_links` clean.
- **Backend (live fixes):** **D2a/D2b editor-shell doc-honesty** (`editor_fs/mod.rs` + `commands.rs`) — narrowed the symlink-rejection claim to leaf-vs-non-leaf granularity + noted `root` is frontend-trusted; hardening stays Deferred. **`STALE-PIP-TOGGLE-DOC-REFS`** (`pip/commands.rs` ×3) — repointed `pip_toggle`→`pip_set_mode`/`pip_set_visible`, dropped the broken `[pip_toggle]` intra-doc link. **`PIPMOVE-COMMENT-INACCURATE`** — distinguished wrapper `set_content_size` vs raw `msg_send! setFrameOrigin:`. **m3-wp4 #1 `start_broadcaster` Result-contract** — trimmed the doc to match the `JoinHandle<()>` sig (no error channel; guard lives at the lib.rs call site). **`APPLY-UPDATE-COMMENT-OVERSELL`** (`tray/commands.rs`) — trimmed the "callable from tests' shape" overclaim. **`lib.rs:258` sublime "removed at WP8"** — rewrote to the permanent-escape-hatch reality (WP8 redefinition).
- **Backend (STALE PREMISES — already corrected, no change):** `CC-KILL-SIGTERM-COMMENT-DRIFT` (the `cc_kill` doc already says SIGKILL — zero "SIGTERM" strings remain). `STALE-FILE-BASE-DOCLINK` (`git_diff/mod.rs` already links `[file_hunks_core]`, no `file_base_core`). `WRONG-DIFF-API-COMMENT` (doc already says `diff_index_to_workdir` for the unstaged path).
- **Frontend (live fixes — Theme D rationale consolidation):** `TRIPLICATED-EFFECT-RATIONALE` (`Workspace.tsx` — trimmed the 11-line `visible`-edge comment to the WKWebview-rAF non-obvious bit + commit/WIP pointer). `RAF-FOCUS-DUPLICATION` (`XtermPane.tsx` — made the mount site canonical, the post-spawn + on-active sites back-reference it). `COMMENT-TRIPLICATION` m2-wp3a (`EditorPanel.tsx` useMemo → back-reference to the `languageOverrideId` doc). m2-wp13 #1 (`PaneTabs.tsx` `closeActiveTabRef` → back-reference to the reporter-ref pattern). `COEXISTENCE-COMMENT-DUP` (`App.css` F12 block → back-reference to the WP4b half-accent block). `STALE-AWAITING-SCALE-COMMENT` (`pipFanoutWiring.test.ts` — "glows the dot, no size scale; operator dropped scale in P4.2").
- **`tooling/demo/` (live fixes):** `M8WP4-DEAD-FOCUS-FLAG` + `DEAD-FOCUS-CSS` (WP1-deferred) — **deleted** the dead `pipFocused`/`row.focused` flag uses (`shell.js`) + the orphaned `.pip.focused` CSS (`shell.css`). `DUP-CURSOR-COMMENT` (`timeline.pip.js` — deleted the duplicated cursor-track comment block). `SMOKE-TIMELINE-ONE-SYSTEM-NAMING` (`timeline.smoke.js` — recast api-gateway/web-client/infra-tf/docs-site → tax-cruncher/recipe-box/blog-engine/catan-companion, 4 unrelated projects per the parallelism philosophy). `README-DURATION-DRIFT` + `COMMITTED-GENERATED-CSS-UNDOCUMENTED` (`tooling/demo/README.md` — example duration → `3.2` per-timeline + the committed-generated-CSS drift-guard note). `SHELL-INNERHTML-RAW-MARKUP` (`shell.js` TIMELINE-shape doc — raw-HTML caution for scenario authors).
- **`tooling/demo/` (STALE PREMISE):** `STALE-JSDOC-CAST` (`timeline.filmstrip.js` JSDoc already names the recast cast catan-companion/tax-cruncher; zero api-gateway/web-client refs).
- **NOT in scope this WP (correctly left for WP6):** the `lib.rs:333` "Replaces the old pip_toggle" note (accurate historical context, not a broken link); the Theme-A/B/E/F/H/I dedup-extraction MINORs (WP6); the tray `TRAY-ID-MATCH-DUP`/`TRAY-ID-UNUSED-LOOKUP` (not in the WP5 list). **Resolved 17 findings** (13 live fixes/deletions, 4 confirmed-already-resolved).
