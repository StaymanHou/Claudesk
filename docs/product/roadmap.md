...
### Milestone 10.5: QoL polish bucket (before the next release) *(NEW — inserted 2026-07-16; an OPEN collection bucket, like the completed M6 friend-QoL)*
...
- [ ] **Active-close confirmation.** A confirm popup before closing a workspace's CC, a terminal, or the whole app when something is **active** (CC `Running`/`AwaitingInput`, or a terminal with a running command) — to prevent mis-click loss. **Extends the existing QoL-WP1 close-confirm gate** (`App.tsx` `dirtyProbes`/`requestClose` + `ConfirmModal`), which today fires only on unsaved-editor-dirty; adds a busy/active probe to the same gate (+ the app-level `CloseRequested` consults the aggregate). *(Spec decides the terminal-"active" definition + whether app-quit lists which workspaces are busy.)*
- [ ] **Proper CC / terminal kill with a clean exit.** Sessions must reliably exit (no hang, no orphaned child). The kill path exists (`cc_session/mod.rs` `kill()`: `/exit\r`→3s poll→SIGKILL); this is **reproduce-first** — capture the symptom (which session kind hangs? `/exit` ignored mid-turn? SIGKILL orphans a subagent? PTY not reaped?) via `/feature-reproduce` before fixing.
- [ ] **CC input/output encoding fix (mojibake, e.g. `-�.`).** No garbled characters in CC I/O. Bytes are base64 (byte-exact) across the boundary, so the prime suspect is a **multi-byte UTF-8 sequence split across PTY read chunks** decoded to a string on the frontend before `term.write` (xterm handles `Uint8Array` writes safely; string writes of a split char corrupt). **Reproduce-first** — find a reliably-mojibaking glyph, then fix at the FE decode path (`XtermPane.tsx`) and/or the Rust reader chunk boundary (`cc_session/mod.rs`).

**Exit Criteria:** PiP summons to the top-right corner; closing an active CC/terminal/app prompts a confirm; CC and terminal sessions always exit cleanly on kill (no hang/orphan); no encoding mojibake in CC input or output. Verified on the installed `.app` (this touches lifecycle + external-process I/O — installed-build smoke per convention).
