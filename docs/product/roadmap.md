...
### Milestone 7: Menu-bar status item *(was Milestone 6 → M7; SHRUNK to ambient alarm + actuator 2026-06-29)*
...
**Deliverables:**
- [ ] **Menu-bar tray icon — 2-state ambient alarm** via `tauri::tray::TrayIconBuilder` (`icon_as_template` for light/dark; atomic `set_icon_and_icon_as_template_atomic` swap). **Lit when ANY workspace is `AwaitingInput`; neutral otherwise** (Running + Idle collapse — running-vs-idle detail lives in PiP / the main window, not the menu bar). Reduced by a pure `aggregate_alarm` fold over the open-workspace states; subscribes to the existing M3 `status_broadcaster` `workspace-status` event (no broadcaster change).
- [ ] **Native actuator menu** (shown on click): Show Claudesk Window / Toggle PiP / Quit — each wired to its existing app action via the 2026-06-24 `app_menu` bridge. Actuators are the non-redundant complement to the alarm (display-only PiP can't act on the app).
