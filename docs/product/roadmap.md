...
### Milestone 8: Demo assets — filmstrip & PiP value showcase *(new — inserted 2026-06-29b; the next execution milestone)*
...
**Deliverables:**
- [ ] **WP1 — Capture/render pipeline probe + format decision (FIRST; gates the rest).** Establish *how* an agent can produce these assets at all on macOS: drive the installed `.app` into the demo scenario, capture the screen (or a window region) to video, and render to the chosen deliverable format. Decide the format (GIF for inline README autoplay vs. MP4 for fidelity/length vs. both) as the WP1 output, grounded in what the pipeline can actually produce. Resolve: which capture tool (`screencapture -v`, ffmpeg avfoundation, or another), how the agent triggers + stops a timed recording, how to render (`ffmpeg`/`gifski`), target dimensions/length/file-size budget for README embedding, and whether agent-driven staging via the MCP bridge is sufficient to choreograph a believable multi-project scenario or whether some operator setup is needed. **Probe outcome documented; GO/NO-GO + chosen format recorded before any final asset is produced.**
- [ ] **Filmstrip demo** — a recording showing ~4 CC-driven projects open as workspaces, the filmstrip roster with live mirrors + status dots, attention shifting as a project flips to awaiting-input, and a tile click promoting it to center stage. Scenario staged on the scratch repos (`tmp/scratch/scratch-{a,b,c}` + one more) or representative projects.
- [ ] **PiP demo** — a recording showing the PiP panel pinned always-on-top while the operator works in another app (deep-focus work / YouTube), CC progress visible in the corner, a status dot flipping to awaiting-input as the monitorable signal.
- [ ] **Embed + place** — the final assets committed to the repo (path TBD per WP1's file-size outcome) and embedded in `README.md` (and available for the GitHub release page / open-source launch).

**Exit Criteria:** Two demo assets exist in the chosen format, agent-produced, each legibly conveying its value prop (filmstrip = parallel-project attention; PiP = monitor-while-elsewhere), embedded in the README and operator-approved at the verify checkpoint. WP1's pipeline probe produced a documented GO + format decision before the final assets were rendered.
