...
**Deliverables:**
- [ ] **PiP default position = top-right corner.** The PiP NSPanel opens at the screen's top-right (computed from `visibleFrame`), not the current ~center default. Preserve a later drag position; anchor top-right only when the panel has no prior position. *(`pip/commands.rs` — `set_content_size` sets size only today; add an initial `setFrameOrigin:`, main-thread-marshaled.)*
