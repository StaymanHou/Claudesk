...
- [ ] **Filmstrip** along the top of the window, populated with one tile per non-center-stage workspace. Each tile shows project name + idle/running/awaiting-input status dot. **Tile body is a live ~1 fps terminal mirror** (WP4 probe PASSED, 2026-06-17) — via `serializeAsHTML()` from the off-viewport background terminal's buffer, NOT an off-screen-DOM mirror (see `wp4-thumbnail-probe-outcome.md`). Clicking a tile promotes that workspace to center stage and demotes the previous one to the filmstrip.
