...
3. **⚠️ The OFF-invariant guard has SIX arms / EIGHT subjects** (M13.5 WP4 added arm 6, as the guard's header predicted; the pin asserts `armSubjects.length === 8` plus a second count derived from the test file's **own source text**). ⚠️ **A new gated surface therefore owns the SEVENTH arm — `arch.md`'s "arms 1–5 are TAKEN" line is STALE** (found 2026-09-07; M15's supervisor is the likely author of exactly such a surface). **Probe each INDIVIDUALLY** — a composite bypass trips *some* arm and hides a gap. ⚠️ Arm 6 proves a *derivation*; `workspaceDriveModeRender.test.tsx` proves the **caller**. Both needed — that split is the recurring defect shape here. ⚠️ The guard scans `src/**` only, **deliberately** (backend OFF is fail-closed Rust-side) — do not "fix" it to reach into `src-tauri/`.
