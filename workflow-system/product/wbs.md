...
**THE VERDICT — a TINY FIXED SET.** ⚠️ **The SHAPE is settled; the MEMBER COUNT moved 5 → 6 at WP2 plan time (2026-08-14).** As decided: `/session-start` · `/session-capture` · `/util-prune-claude-md` · `/util-backlog-paydown` · **`/session-restore`** (added at plan time — see the superseded-exclusion block below) · **Recycle Session** (WP3). The original five-member verdict text follows, still authoritative on *shape* and on why a per-skill registry was rejected. ⚠️ **Five, not "3–5"** (tightened 2026-08-14 at code-quality review): the "3–5" phrasing came from the option text the decision was made *from*, not from the decision — the operator selected this determinate list. **No member is conditional**, and the three low-frequency ones are in **deliberately** (see the ≤3-invocations note below), so dropping any is a scope reduction requiring its own decision, not an implementation detail. ⚠️ A 6th is under discussion — see the open question below — but the *floor* is these five.

- ⚠️⚠️ **SUPERSEDED 2026-08-14 at WP2 plan time — `/session-restore` IS IN THE SET, as member 5 of 6.** The exclusion below is kept because its *reasoning* was reasonable and its refutation is the instructive part; **do not act on it.** Authority for the members is now `workflow-system/state/wip/m13-wp2-skill-buttons.md` → **D3**.
  - **What refuted it:** the exclusion rested on "M12 auto-fires it, so the operator does not type it." A measurement over all 2474 transcripts (523 operator-typed invocations, **97.9% fired with `.session.md` PRESENT**) was expected to *confirm* that — and instead showed the invocations are overwhelmingly **`/clear` then `/session-restore` inside an ALREADY-OPEN session** (`/clear` = 333 typed). ⚠️ **Open-time auto-fire never runs in that flow — the workspace does not re-open.** So the 97.9% describes a population the automatic arm does not serve. The two populations are **not separable from the transcripts**, so the operator decided rather than the figure.
  - ⚠️ **The 92%/531 figure itself was imprecise:** it requires counting **two** command names — `/session-resume` (390, pre-rename) + `/session-restore` (142, post-2026-07-21) = 532 raw, **minus 9 `ScheduleWakeup` self-fires** = **523 operator-typed**. A re-measurement grepping only `/session-restore` finds 142 and would wrongly conclude the earlier figures were inflated.
  - **Accepted cost:** on the workspace-open path the button partially overlaps M12's automatic arm — two mechanisms serving one skill, the shape this WBS calls "the problem". Accepted knowingly, because the flow it serves (post-`/clear`, file present) has **no** affordance at all.
- **The ORIGINAL (now-refuted) exclusion, for the record:** *"`/session-restore` is deliberately EXCLUDED despite being 92% — it is **already automatic**: M12's open-time auto-fire + announce injects it when `.session.md` is present (`predictAction.ts:140`), so the operator does not type it. A button duplicating an *automatic* path is the redundancy `[PRIOR: new-surface-must-earn-its-place-against-existing-ones]` forbids."*
...
✅ **RESOLVED at WP2 plan time, 2026-08-14 — the answer was YES, and the measurement this paragraph mandated is what produced it (see the superseded-exclusion block above; full record in the WP2 WIP → D3).** The original framing is kept below because it prescribed the right instrument.
...
**Tasks:**
- [x] 2.1 ✅ **DECIDED 2026-08-14 → option (i), NO SCANNER; this task is DELETED per its own instruction** (do not build a scanner nothing consumes). Operator-confirmed. The reason is architectural, not convenience: **§4c** (`arch/workflow-gate.md:30`) makes the **command name** the only sanctioned cross-repo coupling, and ⚠️ **the `workflow_substrate` module already refused this very check** — *"a hardcoded roster of skill filenames would be exactly the brittle coupling that clause forbids"* (⚠️ **attribution corrected 2026-08-14 at code-quality review**: that is **`SKILLS_SUBPATH`'s** doc comment, `workflow_substrate/mod.rs:48`, not `skills_dir_exists`'s — which carries the same §4c argument in its own words at `:113`). A 4-path existence check **is** that roster. A missing skill makes CC print "unknown command", and the terminal is the evidence (the same reasoning as `injectCommand`'s no-retry). ⚠️ **Corollary to record: `skills_dir_exists` is NOT these buttons' gate** — "is the substrate installed" and "did the user opt in" are different questions, and wiring the former would make the buttons vanish for a developer install.
- [x] 2.2 Frontend: the **6** fixed buttons (**5** skills + Recycle — see the verdict above; ⚠️ **not "3–5"**, no member is optional), consuming the gate **only** via `useWorkflowFeaturesEnabled` (type-level, executable reference — the guard strips comments). ⚠️ **Placement decided 2026-08-14: the WORKSPACE HEADER**, extending the existing `/session-start` button in place — which is what satisfies "exactly one `/session-start` affordance" by construction. Rejected: the right-panel tab row; one app-level row. ⚠️ The header also holds the name, the next-open indicator and the split control — **layout at a narrow width is a real risk, not a formality.**
- [x] 2.3 Wire each button through **`injectCommand`** (`autoResumeFire.ts:165`) to the **active** workspace's CC pane — the same seam the existing `/session-start` button uses. ⚠️ **NOT `slash_command_bytes`**: that is Rust-side, is not a `#[tauri::command]`, and no button can reach it (⚠️ **CORRECTED 2026-08-14** — this task previously named it, which would have forced either a stall or a brand-new Tauri command, i.e. the second injection path the next sentence forbids). ⚠️ Never a second injection path. ⚠️ Keep the `.catch` — an unhandled rejection is a silent dead click.
- [x] 2.4 **The fifth guard arm.** Assert the **computed** OFF-state value of the skill-button surface. Follow the row-cell arm's shape; read the file's header first (**26 tests** as of 2026-08-14, per-export chord arm).
- [x] 2.5 ⚠️ **Mutation-prove the fifth arm INDIVIDUALLY** — bypass only this arm, confirm red, confirm the mutant landed in **executable** code (`sed -n '<line>p'` it). ⚠️ An invalid probe and a real hole look identical.
- [x] 2.6 ⚠️ **Prove each rendered button has a live CALLER**, not merely an entry in the set. A test that only asserts set membership is the M12 dead-`/exit` trap. Funnel every send through ONE function and guard *that*.
- [x] 2.7 Apply the `sessionStartButton.ts` disposition from 1.6. ✅ **ABSORBED** — the row IS the `/session-start` button; `SESSION_START_COMMAND` + `showSessionStartButton` were **deleted** (not left exported with no caller — the M12 dead-`/exit` shape), their tests retargeted onto the row.
...
⚠️ **WP3 OWES THE ROW ITS SIXTH AFFORDANCE — this is the obligation WP2 could not encode in a test.** WP2 shipped **five** buttons (`/session-start`, `/session-restore`, `/session-capture`, `/util-prune-claude-md`, `/util-backlog-paydown`) in the workspace header and deliberately **omitted Recycle**, because rendering it early-and-disabled would be a promise about unbuilt work. **WP3 adds the Recycle button to that row** (`src/components/workspace/skillButtons.ts` + its render site in `Workspace.tsx`). ⚠️ **It does NOT go in `SKILL_BUTTONS`** — that array holds slash commands and *Recycle is not a skill*; it is a sibling affordance in the same row, wired to WP3's operation rather than to `injectCommand`.
  - ⚠️ **A `DECIDED_ROW_SIZE = 6` constant + a test enforcing this was BUILT AT WP2 AND REMOVED at code-quality review — do not rebuild it.** It was unsound in both directions: `SKILL_BUTTONS.length === 6` is *unsatisfiable* given "Recycle is not a skill" (so WP3 would have hit three mutually-contradictory red tests), and its "WP3 has landed" trigger matched only the literal `"recycle-session"` in `src/**` — invisible to a typed-parameter caller or to the in-process **Rust** writer this WP may legitimately use. Full autopsy: `skillButtons.test.ts`'s do-not-rebuild note. **A test cannot enforce a future scope decision; this WBS line is the enforcement.**
