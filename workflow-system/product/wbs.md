...
| M-12 | ⚠️ **`is_turn_start` ALREADY EXISTS** — the sanctioned turn-boundary seam | `event_is_turn_start()` (`status_broadcaster/mod.rs:451` — ⚠️ **stale as of 2026-09-15: now `:502`**; cite by symbol) = `hook_event_name == "UserPromptSubmit"`, surfaced as `WorkspaceStatusUpdate.is_turn_start` (`:292`); `XtermPane.tsx:343` already consumes it **raw** | ✅ **Shrinks WP3 task 3.1 from "build a trigger" to "consume an existing one."** ⚠️ Its own doc states the rule: *read it off the **raw** event stream, never the folded map.* ⚠️ It is turn-**start**; the turn-**end** correlate is `Stop`, which per `[[derived-state-is-not-a-proxy-for-its-event]]` now maps to **both** `Idle` and `BackgroundWork`. |
...
### WP4: Context-pressure recycle at phase boundaries ✅ SHIPPED 2026-09-14 (commit `79c67e5`)
<!-- ⚠️ Hash corrected at M15 WP5 (2026-09-15). Originally recorded as `75ad76d`, which the 2026-09-14 rebase onto `origin/main` rewrote — that hash no longer exists. The rebase was conflict-free; only the SHAs changed. Sibling rewrites: a791c39→ad9069f, a23969f→13e5936, d28255c→676c597. -->
