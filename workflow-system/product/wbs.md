...
## Session Handoff — 2026-07-31 16:10
Handed off. See `workflow-system/state/.session.md` to restore.

**WP1 is closed (1 of 4).** ⚠️ Next session opens with **two operator-directed repairs BEFORE the next WP**: (A) the red `pnpm format:check` on `main` (37 pre-existing files — needs a decision, not an investigation), and (B) `SURFACE-2026-07-31-QUALITY-WP1-PER-ROW-IPC-REFETCHES-DATA-ALREADY-ON-THE-WIRE` (the N+1 WP1 introduced). Then WP2.
