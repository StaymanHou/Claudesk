...
- **A new gated surface that is not a panel / menu-id / chord / row-cell / skill-row owns a SEVENTH guard arm** — ⚠️ **arms 1–6 are TAKEN** (the fifth landed at M13 WP2; **arm 6, WORKSPACE-DRIVEMODE, landed at M13.5 WP4**). ⚠️ **CORRECTED AT M15 WP5 (2026-09-15)** — this line previously read *"a SIXTH guard arm — arms 1–5 are TAKEN … 7 subjects across the 5 arms"*, which went stale when arm 6 landed. ⚠️ **Probe each arm INDIVIDUALLY** — there are **9 subjects** across the 6 arms (arm 4 owns two derivations, arm 5 two predicates, **arm 6 two** — the drive-mode readout at M13.5 WP4 and the supervisor toggle at M14 WP0), pinned by `armSubjects` in `offInvariantGuard.test.ts` → `it("still polices all six registries")`. ⚠️ **CORRECTED AGAIN 2026-09-18 (M14 WP4)** — this clause said *"8 subjects … arm 6 one"* with a line-number citation that had drifted to an unrelated statement; arm 6 gained its second subject at M14 WP0, after the M15 WP5 edit above was written. **Cite the symbol, never a line number** (`cite-code-by-symbol-not-line`). ⚠️ **M15's supervisor does NOT own a seventh arm** — it is headless (zero `.tsx`, zero JSX, no panel/menu-id/chord/row-cell/skill-row registration) and acts only through the already-gated `injectCommand` / `recycleSession` paths; the decision and its reversing condition are recorded in [workflow-supervisor](arch/workflow-supervisor.md) §G (and in `workflow-system/state/archive/m15-wp5-milestone-exit-verify.md`). ⚠️ `WORKFLOW_TERMS` contains neither `"recycle"` nor `"session"`, so a `RECYCLE_SESSION` menu id or panel registers **unseen** by arms 1–3 — **not widened at M15**, because the only `recycle`-named export is the numeric `RECYCLE_TOKEN_THRESHOLD`. → [gate](arch/workflow-gate.md)
