...
## Booked corrections (✅ both RESOLVED — WP2 Phase 3 and WP4 Phase 1)
...
- ✅ **RESOLVED 2026-09-18 (M14 WP4 Phase 1):** M14's "default CLI args for `claude`" Settings line has been corrected at all **6** live sites (`roadmap.md` ×4, `wbs.md` ×2, `CLAUDE.md`, and this file) — ~~it misstates **PiP (shipped M5)** and **permission-mode (shipped M6)** as future work~~; PiP shipped **M5** (a View-menu window command, deliberately not a preference), the permission-mode dropdown shipped **M6** (app-global), and the per-project `--model` override shipped **M11.5 on the picker row, not in Settings**. Corrected at **task 4.2**. ⚠️ **The task named 2 sites; a repo-wide grep found 6** — the fourth `doc-correction-scope-list-is-a-floor` undercount this milestone.
