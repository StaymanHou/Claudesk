...
> **⚠️ CONTRACT-MAY-SHIFT caveat (read before relying on the schema below).** The segment-model schema here is claude-time's **current** shape, captured as a **baseline**. Two forces will revise it:
> 1. **M9 WP3 is a metric-definitions REDESIGN, not a port** (operator flag 2026-07-06; `SURFACE-2026-07-06-M9-RECLASSIFIER-IS-REDESIGN-NOT-PORT`). WP3's `/feature-spec` locks the real definitions of active / reading / thinking / away / tool-time / subagent-time / per-project attribution, and **those definitions supersede this schema wherever they differ** — the `kind` enum, segment tiling, and per-session `tools`/`prompts` rollups may all change.
> 2. **NEW native-signal source (WP2.5)** — `SURFACE-2026-07-06-M9-NATIVE-SIGNALS-BEAT-GAP-INFERENCE`. claude-time INFERS reading/thinking/away from CC-hook gaps + a guessed typing rate + magic thresholds (it had no other signal). Claudesk, being the terminal + window, can OBSERVE the exact gap — window **focus/blur**, real **PTY keystrokes**, exact **workspace-registry attribution**. WP2.5 captures these as a second event source; WP3 fuses both (measure where possible, infer as fallback). This may add/replace `kind`s (e.g. an observed "typing" vs an inferred "thinking") and change per-project attribution accuracy. **Operator caveat: native signals are better, not perfect (blur ≠ always away — e.g. reviewing a CC-`open`ed screenshot) — so the schema's human-state kinds are especially provisional.**
>
