...
**Workflow:** feature
**State:** research COMPLETE (WP3 task 3.1 probe, 2026-08-25) — ⚠️ **the refutation was OVERTURNED; the
mechanism is FEASIBLE.** Awaiting re-spec (WBS task 3.2). ⚠️ **A SECOND probe is now open** — the
shipped affordance still records ZERO markers on a live pane (operator-observed 2026-08-25 at 0/1/2
turns: nothing moves, button dims), so the break is in the SIGNAL path, not the xterm mechanism.
See `## Research — WP3 signal-trace probe`. *(Renamed from `-REFUTED-mechanism` 2026-08-25: the
name asserted a refutation the probe had already overturned.)*
**Created:** 2026-08-22
...
Claudesk's value proposition is that you run many projects in parallel and *switch away while CC
works*. The cost of that proposition is that you are **not watching when the output scrolls by**.
When you come back to a workspace, you cannot find where the current turn started — so anything CC
said early in the turn is effectively lost.

The acute case (named in the backlog entry): the operator layers a **question on top of a workflow
instruction**. CC answers the question first, then proceeds with the task without pausing. By the
time the operator switches back, the answer is buried mid-scroll behind task output. *"I can't find
that answer anymore."*

**This is measured, not hypothetical.** Grounding the spec in real transcripts was a required first
step in the backlog entry; it was run against this project's own 232 transcripts
(`~/.claude/projects/-Users-stayman-Personal-projects-claudesk`, 516 MB, 1857 turns):

| Metric | Value |
|---|---|
| Median turn | **30** events |
| p75 / p90 / p95 | **119 / 257 / 378** events |
| p99 / max | **721 / 1239** events |
| Turns ≥ 20 events | 1182 (**63%**) |
| Turns ≥ 50 events | 707 (**38%**) |

And the specific buried-answer pattern: **114 instances** where the operator asked a question (`?`
in the prompt) whose turn then ran ≥ 60 events. Real anchors, newest-project-first:

- `ad11840e` — *"so why did you stop?"* → answered inside a **1240-event** turn
- `00fe7fd8` — *"Oh, so there's this implication. Would reverting the previous decision resolve this issue?"* → **1017 events**
- `bede855e` — *"really? Why are you pausing again? autopilot mode!!!"* → **787 events**
- `383a9f53` — *"all good. but... I was working on the neo-stayman-assistant project in the afternoon, why the usage not showing up there?"* → **512 events**
- `4d4975f0` — *"generally speaking - approved. But... Does the banner shift the layout?"* → **477 events**

The last two are precisely the question-layered-on-an-instruction shape: an approval plus a
question, where the approval triggers a long task and the question's answer is the thing that gets
buried.

**Problem statement unchanged — re-checked at the P3.verify-self.2 back-loop (2026-08-22).** The
back-loop was NOT triggered by a defect or by a shifted root cause: the verify-self finding was
*unverifiable-on-a-stale-runtime*, mechanically explained (the running process loaded a 5-element DTO
built before the `is_turn_start` change existed). What the loop exists to fix is a **verification**
gap, not a product one — the P3.2 listener wiring had zero automated coverage — so the problem being
solved for the operator is exactly as written above.

## User Stories

- As the operator returning to a workspace after switching away, I want to **jump straight to where
  the current turn started**, so that I can read the turn from its beginning instead of scrolling
  blindly upward hunting for the boundary.
- As the operator scanning a long turn, I want **turn starts to be visibly marked in the terminal**,
  so that I can tell at a glance which output belongs to which prompt.
- As the operator who layered a question onto an instruction, I want to reach **the answer CC gave
  before it moved on to the task**, so that a question asked in passing is not silently lost.
- As a user with the workflow features **off**, I want none of this to change anything, because turn
  boundaries are a plain Claude-Code concept and this must not become a gated surface.

## Acceptance Criteria

The feature is done when:

1. **Turn starts are marked in the terminal buffer.** Each time the workspace's CC session begins a
   turn, a marker is registered at the terminal's current position, and it is visually
   distinguishable in the scrollback (a decoration in the gutter/scrollbar margin, not injected
   text — see Technical Constraints).
2. **A jump affordance moves the viewport to the latest turn start**, and repeat activations walk
   **backward** through earlier turn starts (latest → previous → …), so recovering an answer two
   turns back is reachable.
3. **The marker set survives switching away and back.** Because all workspaces stay mounted and the
   xterm instance is never unmounted, markers persist for the session's life (subject to the
   scrollback bound in AC-6).
4. **The mechanism reads the turn boundary from a signal that identifies the EVENT, not the derived
   state** — never from PTY output text, and never from the collapsed status map. ⚠️ This requires a
   **backend DTO change** (a discriminator field): as built, `state: Running` is emitted by *both*
   `UserPromptSubmit` and `PostToolUse`, so the state alone cannot mark a turn start. See Technical
   Constraints → "the DTO cannot currently express this".
5. **It is honest when it cannot help.** With no marker available (fresh session, or every marker
   evicted), the affordance is visibly inert/absent rather than silently doing nothing or jumping to
   an arbitrary line.
6. **The scrollback-eviction boundary is explicit and correct.** Settled by OQ-1: the foreground
   terminal's `scrollback` rises to **10000** so realistic long turns stay reachable, and the
   mirror's `scrollback: 40` is left untouched. 10000 is still a bound, so the evicted-marker path
   in AC-5 is built and **tested** either way — the raise reduces how often it fires, it does not
   remove the state. ⚠️ **CORRECTED at P3.1:** the mirror's `40` is an option to
   **`serializeAsHTML()`** (last-40-rows of the *same* buffer), **not a second `Terminal` config**.
   So the raise cannot affect the mirror's cost at all — the feared serialize regression was
   structurally impossible, not merely unmeasured. The spec said otherwise in three places.
7. **The gate does not apply.** This surface is **ungated** — it ships for every user, `on` or `off`
   (see Technical Constraints → gate applicability). No sixth guard arm is taken.
8. **The new DTO field's consumers are swept by sibling literal, not by nearest match.** Per
   `[[derived-state-is-not-a-proxy-for-its-event]]`: when the build adds an event discriminator,
   grep every existing consumer of *each* `WorkspaceState` literal (`running`, `idle`,
   `awaiting_input`, `background_work`, `unknown`) to confirm none of them silently changes meaning.
   WP2's CRITICAL was missed precisely because the sweep grepped one literal and was structurally
   blind to a site keyed on another. A regression test pins whatever the sweep finds.
9. `pnpm verify:auto` exits 0; Rust and frontend test baselines rise (from Rust 873 / frontend 2141).

## Out of Scope

- **⚠️ Any new panel, view, tab, side rail, or window.** `[PRIOR: new-surface-must-earn-its-place-against-existing-ones]`
  fires directly and the WBS states it: the terminal already shows this output, so a new surface
  would be a strict subset plus a permanent sync cost. The irreducible non-overlap is **navigation
  within the existing buffer** — that, and nothing more.
- **The "turn digest" / "since you left" summary panel** (backlog solution direction 3). That is a
  second surface *and* it requires summarizing CC output, which means reading output text.
- **Per-turn answer/notable extraction** — detecting "this was an answer to a question" vs. task
  output (backlog direction 2). It requires classifying PTY output, crossing the never-parse-PTY
  line. If wanted later, the honest input is the hook channel, not the screen.
- **The behavior-side fix** (backlog direction 4: make the orchestrator pause after answering an
  inline question). That is a **workflow-system / mccc** change, not Claudesk's — and per the M15
  ownership boundary, intra-turn semantics belong to mccc. It is legitimately a separate concern
  and may be worth filing there.
- **Search within the terminal** (`@xterm/addon-search`). Adjacent and tempting; a different
  feature, a new dependency, and not what the backlog item asks for.
- **Cross-workspace "who has an unread answer?" aggregation** in filmstrip/PiP/tray. A status-surface
  change, out of this WP's scope.
- **Persisting markers across app restarts.** The buffer itself does not survive a restart, so a
  persisted marker would point at nothing.

## Technical Constraints

**The mechanism is native to xterm 6 — no new dependency.** Verified in
`node_modules/@xterm/xterm/typings/xterm.d.ts`: `registerMarker(cursorYOffset?)` (L1147),
`registerDecoration(opts)` returning an `IDecoration` with `overviewRulerOptions` (L1157, L555–612),
`buffer.markers` (L847), `scrollToLine(line)` (L1233), `scrollLines(amount)` (L1211). Installed
addons are `fit` + `serialize` only; **no addon is needed for this** and none should be added.

**⚠️ The turn boundary is already a first-class event — do not invent a signal.** The CC hook channel
delivers `UserPromptSubmit` at exactly the moment a turn begins; `status_broadcaster` already maps it
(`mod.rs:196` → `WorkspaceState::Running`) and already carries the prompt text on the DTO
(`last_output_snippet`, `mod.rs:407`). Nothing needs to be parsed from the terminal.

**⚠️ But the frontend's status path COLLAPSES it.** `useWorkspaceStatus` folds every update into a
`WorkspaceStatusMap` keyed by workspace id (`useWorkspaceStatus.ts:54`), so consecutive events
overwrite and *"a `UserPromptSubmit` arrived"* is **not recoverable from the map**. A per-event
consumer must tap the **raw** `workspace-status` event stream. This is the exact trap recorded as
`[[workspace-status-map-collapses-consecutive-events]]`, whose failure mode is a feature that
**silently never fires**.

**⚠️⚠️ THE DTO CANNOT CURRENTLY EXPRESS THIS — a backend change is required, and this is the
feature's central correctness risk.** `WorkspaceStatusUpdate` (`mod.rs:249–270`) carries
`workspace_id`, `state`, `last_event_at`, `last_output_snippet`, `notification_type` — and **no
`hook_event_name`**. The frontend therefore cannot tell *which* event produced an update. And
`event_to_state` (`mod.rs:194`) maps **two different events to `Running`**:

- `"UserPromptSubmit" => Running` — a turn **starts** (fires once per turn)
- `"PostToolUse" => Running` — a turn **resumes** after a tool call (fires *many* times per turn)

So inferring "a turn started" from `state === "running"` would register a marker on **every tool
call** — hundreds of markers in the 1240-event turn measured above — which is a defect, not a
degradation. This is `[[derived-state-is-not-a-proxy-for-its-event]]` exactly: M13.5 WP2 shipped a
CRITICAL by reading "a `Stop` arrived" off `state === "idle"`, and the sibling-literal sweep is what
catches it. **`last_output_snippet` is not a usable discriminator either** — it is populated for both
`UserPromptSubmit` (`prompt`) and `Notification` (`message`), so its presence does not identify a
turn start.

**The required change:** add a discriminator to the DTO so the consumer matches the **event**. The
in-repo precedent is `notification_type`, added by M13.5 WP2 for exactly this reason and documented
as *"exposed so a surface can show why without re-deriving."* Follow it: a new
`skip_serializing_if`-omitted `Option` field, snake_case verbatim, mirrored in the TS type, plus the
parallel `serde_json::to_value` key-shape contract test the convention requires
(`dto_serde_shape_is_snake_case`). Whether it is `hook_event_name` passed through or a narrower
`is_turn_start` flag is a plan-level call; the constraint is that the **frontend must not re-derive
it from `state`**.

**⚠️ Never infer turn boundaries from PTY output.** Architectural line
(`arch.md` → load-bearing constraints; `arch/status-channel-and-surfaces.md`). Hook channel only.

**⚠️ `scrollback` is the hard bound on any in-buffer mechanism — rising 1000 → 10000 (OQ-1).** At the
current `scrollback: 1000` (`XtermPane.tsx:331`) a p90 turn (257 events) or p95 (378) can emit more
than 1000 lines, so a turn's own start **can be evicted while the turn is still running**. The
operator settled this by raising the foreground bound to **10000**; the background/mirror path's
`scrollback: 40` (`XtermPane.tsx:377`) stays as-is. Eviction remains a **reachable state** at any
bound — xterm disposes a marker whose line leaves the buffer (`IMarker.isDisposed`) — so the
mechanism must treat it as a normal state, not an error.

**⚠️ Decoration, not injected text.** Writing a visible divider into the PTY stream would corrupt the
buffer that `serializeAsHTML()` reads for the filmstrip and PiP mirrors, and would inject bytes into
a stream Claudesk does not own. Use `registerDecoration` (which renders in the terminal's own layer)
and, for the at-a-glance half, `overviewRulerOptions` — a tick in the scrollbar margin, which is
exactly "where did this turn start" made visible without consuming layout.

**⚠️ Do not rebuild an off-screen live DOM mirror**, and be aware the background terminals mount
off-viewport (`left:-99999px`) with a paused renderer and a reduced `scrollback: 40`
(`XtermPane.tsx:377`). Decorations are a render-layer concern; verify behavior on the **foreground**
pane and confirm the background path is unaffected (the mirror must not gain or lose content).

**⚠️ Verification: an observation is only decisive if a broken implementation would answer
differently.** `arch.md` records that WebKit retains `scrollTop` on a never-unmounted hidden node
and clamps out-of-range writes itself — which **silently vacated two live proofs** in a prior
milestone. A "the viewport moved" assertion must therefore distinguish *our* scroll from the
platform's own retention/clamping. Pair any live check with a positive control, and prefer extracting
the scroll-target decision as a **pure function asserted as a value** (jsdom reports
`clientHeight === 0` for visible elements, so scroll geometry must be an injected value, never read
off an element).

**Gate applicability — ungated, and this is the discriminator, not an oversight.** The gate applies
**per arm, by applicability, never by audience size** (`arch.md`; `arch/session-resumption.md`). A
"turn" is a plain Claude Code concept that exists for every user with or without
`~/.claude/skills/`; nothing here reads `workflow-system/`, a skill, or a drive mode. So it ships
ungated and takes **no** guard arm — unlike WP4's drive-mode readout, which is a workflow concept and
does take arm 6.

**Code shape — follow the established precedent.** Pure decision modules with no React/DOM, unit
tested (`closeTerminalChord.ts`, `panelHost.ts`, `filmstripOrder.ts`). The marker bookkeeping and the
"which marker do I jump to next?" walk are pure and should be extracted as such; only the
`registerDecoration`/`scrollToLine` calls and the event subscription are runtime-bound.

**Chord vocabulary.** `⌘⇧`+digit is reserved for workspace/filmstrip switching
(`[[cmd-shift-digit-reserved-for-filmstrip]]`); `⌘⇧O` is free (`sublimeLaunch.ts`); `⌘W`/`⌘⇧E` are
taken. If the affordance gets a chord, pick from what is genuinely free and record it.

**No 3rd-party dependency** — step 2's probe check does not fire. xterm is already a first-class
dependency and the API used is in the installed version.

## Design decision — the mechanism (WP3 gate 3.1)

The WBS's gate 3.1 requires enumerating candidate mechanisms and picking one.

| # | Candidate | Verdict |
|---|---|---|
| A | **Turn-start marker + decoration + jump-backward affordance** (in-buffer navigation) | **CHOSEN** |
| B | "Jump to last turn start" only, no visible marker | Rejected — strictly less, for the same wiring. The at-a-glance half is where the marker earns its cost; without it the operator must *guess* whether the jump landed. |
| C | Per-workspace scroll-position memory (restore where you were on switch-back) | Rejected — solves a **different** problem (resuming your reading position), not "where did this turn begin". `arch.md` notes WebKit already retains `scrollTop` on the never-unmounted hidden node, so most of this exists for free and it does not answer the question asked. |
| D | Turn digest / side rail / "since you left" panel | Rejected — new surface (prior fires) **and** requires reading output text. |
| E | Behavior-side: make the orchestrator not bury the answer | Out of scope — mccc's, per the M15 ownership boundary. Worth filing there. |

**A is chosen** because it is the only candidate that is (i) the irreducible non-overlap the prior
demands — navigation *within* the existing buffer, (ii) driven entirely by a signal Claudesk already
receives, and (iii) buildable with zero new dependencies and zero new surfaces.

## Scope assessment — WP3 gate 3.2

⚠️ **Gate 3.2 asks whether the spec is larger than one WP; the answer is NO — this stays in the
bucket.** The WBS sized WP3 **M–L** and flagged escalation as the *expected* outcome, so this
verdict reverses that expectation and owes its reasoning:

- The mechanism turned out to be **native xterm API + an event tap**, not a terminal-render-path
  change. The WBS's size fear was *"it touches the terminal render path"* — with
  `registerDecoration` it does not; xterm owns that layer.
- The turn signal **already arrives on the hook channel and is already mapped** — no new hook event,
  no hook-script change, no `~/.claude/` touch.
- **No new surface** — the prior cut candidate D, which was the genuinely M–L option.
- Estimated shape: **one additive DTO field** (backend, with the precedent and its contract test
  already established by `notification_type`) + one new pure module (marker bookkeeping + jump walk)
  + one decoration/scroll wiring block in `XtermPane.tsx` + a raw-event tap + a one-constant
  `scrollback` raise + tests. That is `/feature-plan` scale.

⚠️ **Correction against an earlier draft of this spec:** it claimed "no backend work, no new DTO
field." That was **wrong** — see Technical Constraints → "the DTO cannot currently express this". The
DTO carries no event discriminator and two distinct events both map to `Running`, so a frontend-only
build would have marked every tool call. The additive field is small and has a direct in-repo
precedent, so the verdict (stays in the bucket) holds — but the work is **backend + frontend**, not
frontend-only, and the plan must budget for it.

OQ-1 (the eviction boundary) was the one size risk that remained, and it is now **resolved** as a
one-constant change (`scrollback` 1000 → 10000). Nothing outstanding argues for escalation.

⚠️ **If the plan disagrees, gate 3.2 is still live.** This assessment is made from the spec's
vantage; should `/feature-plan` find the wiring genuinely larger than one WP, escalating to its own
milestone remains the WBS-sanctioned outcome and is not a failure.

## Open Questions

*(None open — OQ-1 resolved by the operator at spec review, 2026-08-22. See Asked.)*

## Asked

- **OQ-1 — the scrollback-eviction boundary.** `scrollback: 1000` (`XtermPane.tsx:331`) means a long
  turn can evict its own start line, so the jump would land nowhere on exactly the p90+ turns that
  motivated the feature. Options put to the operator: accept-and-fail-honestly (no memory change) ·
  raise to 5000 · **raise to 10000** · make it a setting.
  **→ ANSWERED: raise the foreground terminal's `scrollback` to 10000.** Rationale: the feature must
  work on the long turns that motivated it; the cost is bounded because background panes keep their
  reduced `scrollback: 40` and only one workspace is center stage at a time; and it is a one-constant
  reversal if memory proves to matter.

  **Consequences for the build:**
  - `XtermPane.tsx:330` `scrollback: 1000` → `10000`. The background/mirror path's `scrollback: 40`
    (`XtermPane.tsx:377`) is **unchanged** — do not touch it; it exists so a backgrounded terminal's
    mirror keeps capturing the newest rows with a paused renderer.
  - AC-6 resolves to branch **(b)**, but honest failure is **still required**, not replaced: 10000 is
    a larger bound, not an unbounded one (the measured max turn is 1239 events and could still
    exceed it). The evicted-marker path from AC-5 must be built and tested regardless.
  - Watch for a **`serializeAsHTML()` cost regression**: the filmstrip/PiP mirror serializes off the
    buffer, and `arch/status-channel-and-surfaces.md` already records an active-CPU p95 caveat
    (~30% on output bursts). The foreground pane is not the mirror source, but confirm the mirror
    tick's cost did not move; the documented mitigations (sub-1fps background rate, coalesced
    serialize, visible-tiles-only) are the fallback if it did.

All other decisions were taken as defaults — listed below.

## Assumed

Defaults taken **without** asking, each cheap to reverse at a verification gate:

1. **Marker granularity = one per turn start** (`UserPromptSubmit`), not per tool call or per
   assistant message. The backlog item asks for turn boundaries.
2. **Jump direction = backward from newest.** Repeat activation walks to older turn starts; reaching
   the oldest is inert rather than wrapping. Wrapping is a surprise in a scrollback.
3. **The affordance is a button in the terminal pane's existing chrome**, plus optionally a chord.
   No new row, no new panel. Placement/appearance is a verify-human question, not a spec one.
4. **Decoration style = a gutter/overview-ruler tick**, colored from the existing token set, not a
   full-width divider line. Cheap to restyle.
5. **Ungated** — reasoned above from the gate's applicability rule; recorded here because it is a
   decision, not a non-decision.
6. **Foreground-only visual verification.** Background panes keep `scrollback: 40` and a paused
   renderer; the requirement there is only that the mirror is unchanged.
7. **No persistence across restarts** (the buffer does not survive one either).
8. **No new chord if the button suffices** — `⌘⇧O` is free if one is wanted; deferring the choice.
9. **Marker cap = none beyond xterm's own buffer bound.** Markers die with their lines; no separate
   LRU.
10. **The prompt text already on the DTO (`last_output_snippet`) is not surfaced** in this WP — it is
    a tooltip-shaped idea with a known dead path
    (`SURFACE-2026-06-22-QUALITY-WP6-SNIPPET-TOOLTIP-DEAD-PATH`) and folding it in would widen scope.
