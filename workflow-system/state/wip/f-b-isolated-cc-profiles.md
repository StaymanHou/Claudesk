...
- [ ] Phase 4: Picker profile cell + Settings "Profiles" group (adopt / remove / missing state)  <!-- status: NOT-STARTED; depends on Phases 2, 3 -->
  ...
  **Observable outcomes:**
  ...
  - [ ] verify-human  <!-- status: in-progress — boundary (picker page, Settings panel); capture = verify-self's live DOM/IPC evidence -->
    - [ ] P4.verify-human.1 Judge the widened picker + profile column (layout/wording)  <!-- status: FAILED 2026-09-24 → operator correction: "make it as a new row above the model selector and the drive mode selector … auto prefix it with 'Profile:'". F12 rework (built, verify:auto EXIT=0 41s, 3040 FE / 967 Rust): the profile is now the FIRST LINE of the model cell's stack (`profileLine.ts`, always prefixed `Profile: `, always present/ungated); the separate column + `ProjectProfileCell.tsx` are DELETED; `PICKER_ROW_CELLS` back to 3; picker reverted to 640px; the model column 9.8em → 12em to fit `Profile: original-dev`. Re-verified live after a fresh relaunch (not HMR — a component was deleted): 0 separate profile cells, 3 row children, picker 640px, model cell 165px, 0 truncated lines on 7 rows; every stack starts `Profile: …`; scratch-a `Profile: original-dev` with no drive-mode line (profile → workflow off); the line opens an in-place select [default, original-dev] with the model/mode lines kept. Awaiting re-judgment. -->
...
- **Path:** F-b > Phase 4 > verify-human (re-entry: P4.verify-human.1 only)
- **Active scope:** P4.verify-human.1 re-judgment (dev build PID 64837 left running for the operator)
- **Blocked:** none
