...
- **Pointer:** **7 findings — 0 CRITICAL, 3 MAJOR, 4 MINOR** (auto-backlogged per `drive_mode=autopilot`). ⚠️ **All three MAJORs are ONE defect viewed three ways: the supervisor is not observable to the operator it acts for.** (1) `onRecycle` announces *"recycling X…"* then calls `fireRecycle`, which **silently returns** when `recycling` is true or the session id is null — and the `FireLedger` has **already claimed the turn** (`fanOut.ts:204`, before the recycle decision at :243), so the turn is **neither fired nor recycled and never reconsidered**: a silent stall whose only log line asserts the opposite. (2) A **successful fire is entirely unlogged** (`outcome.fired` is never read; `injectCommand` logs only on IPC rejection) — the rarer recycle branch got the announcement the common fire path lacks. (3) `wip_read` collapses a **genuine IO error** into the same `empty` as "no WIP file", diverging from the sibling `transcript` module its own header claims to mirror (`wip/mod.rs` preserves the distinction correctly; only the command discards it). ⚠️ **These land in a WP whose five behavioral checks are DEFERRED to dogfooding** (`SURFACE-2026-09-14-SUPERVISOR-NEVER-OBSERVED-FIRING-IN-A-LIVE-SESSION`), so a log line is currently the only evidence a fire or recycle would leave. Full bodies: [`workflow-system/state/backlog-quality-findings.md`](backlog-quality-findings.md) under `# m15-wp4-context-pressure-recycle — 2026-09-14`.
