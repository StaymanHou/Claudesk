...
- **Pointer:** **2 MAJOR + 6 MINOR, 0 CRITICAL** from `feature-review-quality` against `7c91d30^..374e7a4`, auto-backlogged per `drive_mode: autopilot`. MAJOR-1: `check:link`'s "both webview entries" claim is mutation-proven once by hand but not pinned; its fixture test cannot load the repo's `vite.config.ts`. MAJOR-2: the Vitest-reads-`undefined` rationale is duplicated about 8 times. The MINORs are over-stated comments and isolation in `appBoot.test.tsx`, plus a release-skill check that cannot fail. Details: [`workflow-system/state/backlog-quality-findings.md`](backlog-quality-findings.md) → `# paydown-wp3-boot-smoke-test — 2026-09-23`.
- **Priority:** medium (MAJOR-1) / low (the rest)
- **Status:** pending
- **Pickup shape:** MAJOR-1 is about 10 lines in `linkCheck.mjs` (assert that both entry chunks are in the RollupOutput), mutation-proven by narrowing `rollupOptions.input`. MAJOR-2 and the comment MINORs fit paydown WP4 (narrowing over-claiming comments). ⚠️ None is a shipped defect; all are this sweep's central risk class, stated coverage larger than actual coverage.
...
- **Pointer:** **2 MAJOR + 3 MINOR** (was 3 MAJOR; the export-guard MAJOR was resolved at paydown-2026-09-23 WP3 by `pnpm check:link`), auto-backlogged per `drive_mode: autopilot`. **0 CRITICAL** — the review states this *"advances the codebase, with a modest, well-identified debt in guard shape and comment budget"*, so **no refactor pass is required**. ⚠️ The first MAJOR is about the WP's own VERIFICATION work, not the feature. (1) `turnNavControls.test.ts` `?raw`-greps for **DOM-at-rest** questions, which `docs/lessons/source-text-guards.md` explicitly routes to a render test and for which it names **two existing precedents needing no new dependency**; the regexes break on a Prettier reflow and cannot cover gate-OFF. (2) A genuine three-layer **contract drift**: `stepTurn` returns `nav`, `XtermPane` discards it, the handle returns a `boolean` that `Workspace` also discards in favour of a follow-up `turnNavState()` — while comments at two layers assert push-not-poll. Full bodies: [`workflow-system/state/backlog-quality-findings.md`](backlog-quality-findings.md) under `# turn-output-reorientation — 2026-08-25`.
...
- **Priority:** medium (2 MAJOR — guard shape + contract drift, no correctness impact) + low (3 MINOR)
- **Status:** pending
- **Pickup shape:** read the five entries in `backlog-quality-findings.md`, then `/feature-refactor`. To dismiss, edit the `## Code-Quality Review` section in the archived WIP and mark the line `[DISMISSED]`.
