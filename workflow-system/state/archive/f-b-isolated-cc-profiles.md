...
- [x] Phase 5: New-profile wizard + Delete-to-Trash  <!-- status: done 2026-09-24 -->
  ...
  **Observable outcomes:**
  ...
  - [x] verify-auto  <!-- status: done — `pnpm verify:auto` EXIT=0 on `6b8b19d` (39s; 3068 FE / 982 Rust lib; lint's 1 warning is pre-existing in XtermPane.tsx, also at HEAD~1). Mutation-proven at build: Esc window-capture (→document fails), Adopted-delete refusal, create rollback — each mutant confirmed landed -->
  - [x] verify-self  <!-- status: done 2026-09-24 — orchestrator-driven via the `tauri` MCP bridge (bridge does not reach subagents; deviation already recorded in Discoveries). Integration boundary: YES (picker row `ProjectModelCell`, ⌘, Settings Profiles group) — outcomes cite both surfaces by name. Dev app data snapshotted before and restored after (only `projects.json` differed). ⚠️ COSMETIC (not a blocker, for verify-human): the backdrop's `rgba(0,0,0,0.5)` dim is barely visible over the picker's near-black background, so the wizard reads as a floating panel rather than a modal. -->
    - [x] P5.vs.1 Picker row `Profile:` select offers `default · original-dev · New profile…`; choosing it opens the wizard (portalled into <body>, fixed, z-index 60) and commits nothing (row still `Profile: default`)  <!-- status: PASS -->
    - [x] P5.vs.2 Steps walked in order `1/8 Name → 2/8 Config directory → 3/8 Permission mode → 4/8 Log retention → 5/8 Default model → 6/8 Theme and status line → 7/8 Mouse and selection → 8/8 Confirm`; dir pre-filled `/Users/stayman/.config/claude-fbverify`; permission step rendered with `acceptEdits` pre-selected (= `~/.claude` `permissions.defaultMode`); retention `99999`; theme `dark` from CC's 7; status-line step shows `npx -y ccstatusline@latest`, checked; mouse + copy-on-select both unchecked  <!-- status: PASS -->
    - [x] P5.vs.3 Created (`plan` chosen): `settings.json` `permissions.defaultMode=plan`, `cleanupPeriodDays=99999`, `statusLine` deep-equal to `~/.claude/settings.json`'s, `env.CLAUDE_CODE_DISABLE_MOUSE="1"`, `theme=dark`, no `model`, hook marker `claudesk-hook-dev.pl` present (prod marker absent); `.claude.json` = `{"copyOnSelect": false}` exactly; `CLAUDE.md` has the guard heading + the dir. `grep -c claude-fbverify ~/.zshrc` 0 → 0 and `~/.zshrc` shasum unchanged. Listed as `created`; the row from which it was created now stores `"profile": "fbverify"`  <!-- status: PASS -->
    - [x] P5.vs.4 Non-empty dir (`tmp/scratch/f-b-prof/claude-scratch`) → refused at the dir step with "Add … as an existing config dir instead"; the backend refuses independently (`profile_create` invoked directly → "is not empty — add it as an existing config dir instead"); the dir's file-hash digest identical before/after (`d8b4de69…`); nothing listed. Name `original-dev` → "A profile named "original-dev" already exists.", stays on step 1  <!-- status: PASS -->
    - [x] P5.vs.5 First spawn under the fresh profile: child `claude --model opus` with `CLAUDE_CONFIG_DIR=…/claude-fbverify`, no `--permission-mode`, no `CLAUDESK_DRIVE_MODE`. xterm BUFFER (fiber walk, not DOM) shows CC's own onboarding "Let's get started. Choose the text style…" with Dark mode ✔ — the first screen of CC's login flow per probe (1); Claudesk injected nothing. ⚠️ The login screen PROPER is one Enter later; a bridge `press Enter` did not reach the PTY, so that screen is observed at verify-human (the operator logs in there anyway)  <!-- status: PASS (scoped: onboarding observed; login screen → verify-human) -->
    - [x] P5.vs.6 Settings: `Delete…` present ONLY for `fbverify` (created); `original-dev` (adopted) has Remove but NO Delete in the DOM. `New profile…` in Settings opens the wizard; one Escape closes the wizard and Settings STAYS open; positive control: a second Escape then closes Settings  <!-- status: PASS -->
    - [x] P5.vs.7 Delete: confirm reads "/Users/stayman/.config/claude-fbverify will be moved to the Trash, and its conversation history, memory, settings and login go with it…", focus on Cancel. Move to Trash → dir gone from `~/.config`, `claude-fbverify` present in Finder's Trash (queried via Finder scripting — `~/.Trash` itself is TCC-blocked for this shell), `profiles.json` back to `original-dev` only, row gone from the list; the scratch-c row now reads `Profile: ⚠ fbverify missing` (A.6)  <!-- status: PASS -->
  - [x] verify-human  <!-- status: done 2026-09-24 — operator: "verified the work in the dev app. looking all good" -->
    - [x] P5.verify-human.1 End-to-end: create a profile from the wizard (from a picker row), open that row, complete CC's onboarding + `/login`; confirm theme, status line, no mouse capture, no copy-on-select, **and the pane footer shows the profile's permission mode (B.10)**. Agent then reads back `.claude.json` `copyOnSelect` + `settings.json` `theme` post-login (the deferred P1.verify-human.2)  <!-- status: PASS — operator verified in the dev app with profile `test-test`. Agent read-back AFTER login: `.claude.json` `copyOnSelect: false` survived, no `theme` key there; `settings.json` `theme: dark`, `defaultMode: auto`, `CLAUDE_CODE_DISABLE_MOUSE=1`, `statusLine` ccstatusline, dev hook marker present; `oauthAccount` present and `hasCompletedOnboarding: true` written by CC itself (not seeded) -->
    - [x] P5.verify-human.2 Close the workspace, Settings → Profiles → Delete… that profile; confirm the dialog reads right and the dir lands in the Trash (agent re-checks `~/.config` + Finder's Trash)  <!-- status: PASS — operator "looking all good"; the Trash move itself was verified end-to-end by the agent in P5.vs.7. The operator's `test-test` profile is still listed and on disk — left in place as theirs -->
    - [x] P5.verify-human.3 Judgment: accept or correct build defaults (a)–(f) in Discoveries  <!-- status: PASS — accepted as built (operator: all good) -->
    - [x] P5.verify-human.4 Low-priority (verify-self COSMETIC): the modal backdrop's dim is barely visible over the picker — accept or ask for a stronger dim  <!-- status: PASS — accepted as built -->
    - [x] P5.verify-human.5 Installed-`.app` Finder-launch smoke (profile spawn + dot; also: does the Trash move need a Finder Automation grant from an installed build?) — **deferred to the `/release` gate** per the operator's standing preference (G.30)  <!-- status: done (DEFERRED to /release — carry: profile spawn + dot from a Finder-launched .app, and whether `trash::delete` needs a Finder Automation grant there) -->
  - [x] verify-codify  <!-- status: done 2026-09-24 — integration boundary: YES. Added (1) `src/__tests__/invokedCommandsAreRegistered.test.ts` — every literal frontend `invoke("name")` must be in `lib.rs` `generate_handler!` (the gap every mockIPC test is blind to); positive control on the four Phase 5 commands + bare-registered `quit_now`; mutation-proven (dropping `profile_delete` from the handler fails it, naming `src/state/profiles.ts`; landed, restored by shasum). (2) consuming surface `ProjectPicker`: New profile… from a row → wizard → create → `set_project_profile {path, "work"}` → re-read on `profiles-changed` → row reads `Profile: work` and OPENS (`record_open`). (3) Settings: one Esc cancels the Delete confirm, sends nothing, never reaches an App-style document capture listener — mutation-proven (dropping the confirm's `useEscCapture` fails it). Full `pnpm verify:auto` EXIT=0 (45s; 3072 FE / 982 Rust lib) -->
