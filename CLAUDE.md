...
- **Roadmap (resequenced dogfood-first 2026-06-22; +friend-QoL/docs-viewer/time-analytics inserts; demo-milestone insert + time-analytics↔docs-viewer swap 2026-06-29b):** M1–M7 complete; execution order onward is **M8 demo-assets** (current) → **M9 time-analytics** → **M10 docs-viewer** → **M11 auto-resume** → **M12 skill-orchestration** → **M13 polish**. See `roadmap.md` revisions (latest: 2026-06-29b).
- **Published:** origin/main at `c16670b` (v0.2.3, tag pushed); M7 cycle + the v0.2.3 release commits are LIVE on GitHub + the Homebrew tap (`0a5944d`). Releases via the `/release` skill (current: **v0.2.3**).
