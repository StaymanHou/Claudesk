...
- **Sublime hotkey-pop (Phase-1 stopgap):** the real Sublime Text is one keystroke away (`⌘⇧E` in Phase 1; reassigned to `⌘⇧O` in M2/WP5 once `⌘⇧E` becomes the Editor-panel chord) while the in-app editor doesn't yet exist. This is a **temporary bridge, not a permanent escape hatch** — once the in-app lite editor lands (roadmap Milestone 2), the in-app editor *replaces* Sublime **Text** and the pop is removed (the right-half panel-select hotkeys, editor ↔ diff viewer ↔ second terminal, are the navigation that survives). **Sublime *Merge* is the exception — it is KEPT permanently:** a permanent "Open in Sublime Merge" button (`smerge_open`, added in WP5) covers staging/blame/history/blob-at-rev that the inline diff viewer does not. See `docs/product/vision.md` Core Principle 3.
...
- **One window, many workspaces.** Claudesk is single-window. Multiple projects open simultaneously = multiple workspaces inside that one window, switched via filmstrip tiles. Multi-window for Claudesk itself is explicitly out of scope. The standing auxiliary surfaces are: the PiP NSPanel (Phase 2 conditional) and the menu-bar popover (Phase 2). The Phase-1 Sublime **Text**-popped window is a **temporary stopgap removed once the in-app editor lands** (roadmap Milestone 2) — not a permanent auxiliary surface. **Sublime *Merge* is kept permanently** (launched via a persistent "Open in Sublime Merge" button, `smerge_open`, added in WP5) — it is the exception to the Sublime-pop removal.
...
- **Sublime hotkey is in-app, not OS-global (WP8, 2026-06-19).** `⌘⇧E` is a webview `keydown` handler on the focused workspace + an "Open in Sublime" right-panel button — both call the backend `sublime_open` command. The original OS-global `tauri-plugin-global-shortcut` design (with a macOS Accessibility onboarding flow) was built then rejected at verify-human in favor of in-app. No Accessibility permission required. **Temporary by design (revised 2026-06-19):** this Sublime **Text** pop is a Phase-1 stopgap — once the in-app lite editor lands (roadmap Milestone 2), the in-app editor *replaces* Sublime Text and the pop (`⌘⇧E`→`⌘⇧O` handler + "Open in Sublime" toolbar button + `sublime_open` backend command) is removed at WP8. **Sublime *Merge* is NOT removed (revised 2026-06-20, WP5):** a permanent "Open in Sublime Merge" button + `smerge_open` command (added in WP5) stay past WP8 — the inline diff viewer covers *viewing*, but staging/blame/history/blob-at-rev live in Sublime Merge. See `docs/product/vision.md` Core Principle 3.
