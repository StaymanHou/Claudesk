...
- **Code style.**
  - **⚠️ `pnpm verify:auto` IS the per-phase verify-auto gate — run that one command, not a remembered list.** Added 2026-08-18 (paydown WP5) to close `SURFACE-2026-08-01-NOTHING-ENFORCES-FORMAT-CHECK`: there is no CI and no git hook, so a gate spelled out only as prose is a gate that gets partially skipped. It runs, in order: `lint` → `format:check` → `tsc --noEmit` → `vitest` → `cargo fmt --check` → `cargo clippy --all-targets -D warnings` → `cargo test`. Proven to exit non-zero on **both** a Prettier violation and a `cargo fmt` violation. ⚠️ **`cargo fmt --check` is in there deliberately** — the Rust side had the identical unenforced gap (found at WP1, when `cargo fmt` silently reformatted a file that task never touched), so both halves are one command now rather than two passes.
...
- **Tests.**
  ...
  - **Live verify-self via the `tauri` MCP bridge — prefer it over carrying visual/DOM checks to the operator.** The bridge attaches to the real running WKWebView (and the PiP panel) with live Tauri IPC. ⚠️ A dozen-odd caveats (⚠️ **read the doc for the list — a count here goes stale**, and this line already did: it said "Ten" while the doc carried eleven), several of which have produced false verdicts — including (h) **a freshly-opened CC pane reads blank for seconds and it means nothing**, (i) **the xterm DOM is not the buffer, so a working pane reads as 1–3 characters** (and `innerText` on the pane returns xterm's injected *stylesheet*), and (l) **`__TAURI_INTERNALS__` has no `invoke` property to patch, so a JS-level IPC tap logs zero calls for a run that demonstrably wrote** — note the asymmetry: *calling* `__TAURI_INTERNALS__.invoke(...)` works and the doc recommends it; only patching-to-observe fails. ⚠️ Instrument **agreement is not correctness** when both instruments share a defect — run a positive control. ⚠️ **Never conclude "no IPC happened" from a JS-level tap without a positive control.** Read `docs/lessons/mcp-tauri-bridge-caveats.md` before driving it.
  ...
  - **⚠️ Verification hygiene — the single biggest source of defects here.** **Guards that report green while checking nothing**: a numbered catalogue (⚠️ **read the doc — a count here goes stale**, and this line already did: it said "eight" while the doc carried eleven), each found by mutation rather than review, covering `?raw`/`include_str!` source guards AND filtered test runs (`cargo test <filter>` matching zero tests prints `ok. 0 passed` and **exits 0** — pin a count). The test before trusting any of them: *could this still pass if the code it names were deleted?* ⚠️ **Entry 11 extends this to a BEHAVIORAL assertion**: an ordered effect-log is blind to any step that emits no observable — a bare `sleep()` logged nothing, so a five-effect ordering assertion could not see which side of an `await` it fell on, and v0.3.3 shipped the settle on the wrong side with the suite green. Its companion question: *which steps in this sequence can actually APPEAR in what I am asserting over?* **Comment budget**: keep the invariant at the code, move provenance to the archive — density was flagged in *four consecutive reviews* of one file because each trimmed a little and moved on, and duplication is the expensive half (the same rationale in six places drifts asymmetrically). Both, plus the mutation method (prove each form INDIVIDUALLY; confirm the mutant landed in executable code; an invalid probe and a real hole look identical): `docs/lessons/source-text-guards.md`.
