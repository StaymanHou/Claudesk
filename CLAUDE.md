...
- **One window, many workspaces.** Claudesk is single-window. Multiple projects open simultaneously = multiple workspaces inside that one window, switched via filmstrip tiles. Multi-window for Claudesk itself is explicitly out of scope. The only auxiliary windows are: the PiP NSPanel (Phase 2 conditional), the menu-bar popover (Phase 2), and Sublime windows the user pops via hotkey.
