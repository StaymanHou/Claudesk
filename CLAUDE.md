...
- **Tests.**
  ...
  - **⚠️ Verification hygiene — the single biggest source of defects here.** **Guards that report green while checking nothing**: eight distinct ways, each found by mutation rather than review, covering `?raw`/`include_str!` source guards AND filtered test runs (`cargo test <filter>` matching zero tests prints `ok. 0 passed` and **exits 0** — pin a count). The test before trusting any of them: *could this still pass if the code it names were deleted?* **Comment budget**: keep the invariant at the code, move provenance to the archive — density was flagged in *four consecutive reviews* of one file because each trimmed a little and moved on, and duplication is the expensive half (the same rationale in six places drifts asymmetrically). Both, plus the mutation method (prove each form INDIVIDUALLY; confirm the mutant landed in executable code; an invalid probe and a real hole look identical): `docs/lessons/source-text-guards.md`.
...
**Milestone 13: Skill orchestration** — ▶ **IN PROGRESS.** 4 WPs at `workflow-system/product/wbs.md`: **WP1 probe** (M) ✅ SHIPPED 2026-08-14 · **WP2 skill buttons + the 5th guard arm** (S, was M) ✅ SHIPPED 2026-08-14 · **WP3 Recycle as a callable operation** (L) ◀ NEXT, and the critical path · **WP4 exit verify + Group C close** (S). Goal: common workflow operations are clicks, not typed slash commands. **Next: `/feature-plan` for WP3.**

⚠️ **What WP2 shipped, and the two things it hands WP3:**
- **Five** skill buttons in the **workspace header** (`src/components/workspace/skillButtons.ts` → `SKILL_BUTTONS`), all firing through **one funnel** (`fireSkillCommand` → `injectCommand`, label `"skill-button"`). The standalone `/session-start` button was **absorbed** — `SESSION_START_COMMAND` and `showSessionStartButton` are **deleted**, not orphaned.
- ⚠️ **WP3 OWES THE ROW ITS SIXTH AFFORDANCE (the Recycle button).** It does **NOT** go in `SKILL_BUTTONS` — that array holds slash commands and *Recycle is not a skill*; it is a sibling in the same row wired to WP3's operation. ⚠️ **A `DECIDED_ROW_SIZE` constant + test enforcing this was built and REMOVED at review as unsound in both directions — do not rebuild it** (autopsy in `skillButtons.test.ts`; the enforcement now lives in `wbs.md` → WP3). **A test cannot enforce a future scope decision.**
- ⚠️ **`/session-restore` IS in the button set** — WP1's exclusion was **reversed at WP2 plan time** on a measurement that inverted its own expected reading (97.9% of 523 typed invocations had `.session.md` present, but they are overwhelmingly `/clear`-then-restore in an **already-open** session, which open-time auto-fire never serves). Do not "restore" the exclusion. Full record: the WP2 archive → D3.
- The OFF-invariant guard now has **FIVE** arms (the fifth is the skill row; it asserts companion-skill **provenance** by command prefix, deliberately *not* by widening the shared `WORKFLOW_TERMS`). Test counts at this close: **827 Rust lib + 2052 frontend**.
...
3. **Reusable pieces already exist; do NOT build new ones.** ⚠️ **A BUTTON injects via `injectCommand`/`slashCommandPayload` (`src/components/workspace/autoResumeFire.ts:165`/`:145`) → `invoke("cc_input", …)`** — that is what the existing `/session-start` button and all of M12 WP3 use. ⚠️ **`slash_command_bytes` (`cc_session/mod.rs`) is the RUST-side CR rule and is NOT reachable from the frontend** (not a `#[tauri::command]`; one production caller, the shutdown `/exit`); `slashCommandPayload` is its deliberate TS mirror, pinned byte-for-byte by `autoResumeFire.test.ts`. **Two implementations of one rule is intended — keep them in step, do not "unify" them.** *(⚠️ Corrected 2026-08-14 at M13 WP1's code-quality review: this line previously called `slash_command_bytes` "the PTY-injection primitive," which would have sent WP2 to a function no button can call.)* `sessionStartButton.ts` fires **`/session-start`** (not `/session-restore`) and is the single-button precedent. `session_state::consume` returns-and-clears. ⚠️ **M13 reuse of the drive-mode signal is NOT pre-committed** (operator: *"I'll need to open the spec and re-evaluate if it's reusable when we get there"*) — Recycle re-spawns CC, so it gets the signal free from the ordinary spawn path **if** the mode is still set; record what exists, do not abstract for an unspecced caller.
