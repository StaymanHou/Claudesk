...
**Milestone 13.5 — QoL polish bucket.** Inserted 2026-08-19 at the clean boundary after the backlog-paydown sweep closed 8/8; decomposed in `workflow-system/product/wbs.md`. The fourth bucket of its kind (M6 · M10.5 · M11.5, all closed at 4 WPs) — ⚠️ **but this one closes at FIVE**: WP3 escalated out on a mechanism refutation and was **re-admitted the same day** when its own probe overturned that refutation. **WP1 ✅ · WP2 ✅ · WP3 ✅ shipped; WP4 (drive-mode readout on the workspace surface) is next, then WP5 (bucket exit verify).**

⚠️ **One thing from WP3 that must not be re-derived — it cost a blank app and a voided verify cycle:** a **deleted ES-module export is a RUNTIME failure, not just a `tsc` error**. A phase plan accepted a red `tsc` across phases and called it "the guard"; the webview threw `SyntaxError: Importing binding name … is not found`, `main.tsx` never mounted, and the previous phase's "live" verification had been reading a **pre-deletion bundle** the whole time. ⚠️ **Never split an export deletion from its consumer migration across phases, and run a boot smoke-test (`#root` has children) before trusting any live observation on a phase that deleted anything** — `pnpm verify:auto` and `tsc` were *both green* while the app was blank.
