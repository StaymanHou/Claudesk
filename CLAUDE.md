...
**Milestone 8: Workflow-docs markdown viewer.** *(Next execution milestone — not yet planned. M7 menu-bar status item is COMPLETE + CLOSED, see status below.)* A right-panel tab that renders the workflow's strategic markdown docs (`roadmap.md` → `wbs.md` → the active `wip/*` → `backlog.md`) for the focused workspace — an attention/re-orientation surface (NOT a general doc reader): "where does this project sit in the workflow right now?" See memory `m7-docs-viewer-intent` (the intent note predates the M6→M7 renumber; it's the M8 docs-viewer). The re-anchored workflow-doc-hierarchy watcher (`SURFACE-2026-06-22-WP5-DROPPED-WATCH-WORKFLOW-DOC-HIERARCHY`) lands here — the rendering counterpart to the watch seam. **Not yet decomposed** — run `/session-start` (or `/product-roadmap`→`/product-wbs`) to plan it.

**Status (2026-06-29):**
- **Milestone 7 (Menu-bar status item — ambient ALARM + ACTUATOR, shrunk) — COMPLETE + CLOSED** 2026-06-29 (`/feature-finalize` → `/product-finalize`). All 3 WPs shipped in commit `3888dd6` (WP1 tray icon + 2-state ambient alarm: `aggregate_alarm` pure fold over the M3 `workspace-status` states, lit when ANY workspace `AwaitingInput` else neutral, template glyph = monochrome app-icon portrait + lower-right corner badge, swapped blink-free via `set_icon_with_as_template` [main-thread-marshaled internally]; WP2 native actuator menu Show Claudesk / Toggle PiP / Quit wired backend-side through the existing app-level `on_menu_event`; WP3 milestone-exit verify). **NOT a third status surface** — the popover dashboard (per-workspace list + navigate + `tauri-plugin-positioner`) was CUT at the spec debate as a strict PiP subset (design-prior `new-surface-must-earn-its-place-against-existing-ones`). New `src-tauri/src/tray/` module + `tray-icon`/`image-png` tauri features + 2 template PNGs; NO new dependency/webview/Vite entry; NO `trayIcon` config block (glyphs embedded via `include_bytes!`). Glyph + all 4 menu actuators operator-approved live in `pnpm tauri:dev`; native badge-transition + out-of-focus/installed-`.app` checks carried DEFERRED-TO-RELEASE (OC.1–OC.4). 3 MINOR code-quality findings auto-backlogged. WBS archived at `docs/product/archive/milestone-7-menu-bar-status-item/`. **Not yet released** (commits local at `a2f9fe6`; next `/release` cuts the version).
...
- **Roadmap (resequenced dogfood-first 2026-06-22, +friend-QoL/docs-viewer/time-analytics inserts):** M1–M7 complete; execution order onward is **M8 docs-viewer** (current) → **M9 time-analytics** → **M10 auto-resume** → **M11 skill-orchestration** → **M12 polish**. See `roadmap.md` revisions.
- **Published:** origin/main was at `1d01eb9` (v0.2.2); M7 cycle commits are LOCAL (latest `a2f9fe6`) — not pushed/released yet. Releases via the `/release` skill (current: v0.2.2).
