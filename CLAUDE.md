...
⚠️ **M15 AS-BUILT LIVES AT `workflow-system/product/arch/workflow-supervisor.md`** (written at WP5, 2026-09-15) — the typed policy graph, turn-end detection, the `claude -p` adjudicator, context-pressure recycle, observability, and the mccc ownership boundary. **Read it before touching the supervisor.** Four things it settles that must NOT be re-derived: the cell vocabulary is **FIVE values, not a boolean** (`AUTO`/`PAUSE`/`AUTO-SKIP`/`SKIP`/`n/a`, plus Mode 0 outside the four); policy rows key on **STEP, not transition ID** (19 of 27 feature rows), so edge→row is derivation work; `verify-human` is the matrix's **ONE conditional cell** and `resolveCell`'s fallback to **`pause`** is a safety property (a fire there is read by CC **as the operator's reply**); and the supervisor is **headless — it owns NO guard arm**, so the OFF-invariant pin stays at 6 arms / 8 subjects.

⚠️ **M15 IS CLOSED — cycle-finalized 2026-09-15, all 5 WPs shipped.** `wbs.md` is archived to `workflow-system/product/archive/milestone-15-workflow-supervisor/`. **The next milestone is M14's remainder** (Settings UI, Developer-ID signing + notarization, two-tier setup docs, repo metadata). ⚠️ **The milestone's core behavior has NEVER been observed acting** — WP4's five behavioral checks and WP5's live-observation phase are **DEFERRED-TO-DOGFOODING by operator decision, not passed**; an agent cannot manufacture them (an agent-launched CC emits no hook events). **Read the `DEFERRED-*` status tags, NOT the `[x]` checkboxes.** M15 closes with its exit criterion **explicitly unmet rather than falsely ticked** (`SURFACE-2026-09-14-SUPERVISOR-NEVER-OBSERVED-FIRING-IN-A-LIVE-SESSION`, high). ⭐ **`v0.5.0` WAS CUT 2026-09-15 and CLEARS the structural blocker on that item** — the installed app was `v0.4.0` and predated every line of the supervisor, so the six checks were *unsatisfiable*; they are now **satisfiable but NOT yet satisfied.** ⚠️ **The operator upgrades only when all mid-flight tasks reach a clean boundary** (a `brew upgrade` kills every running Claudesk) and then needs **~1 week of real use** — so expect this to stay open for a while, and do NOT read the release as having closed it.

**Milestone 15 — Workflow supervisor. ✅ CLOSED 2026-09-15** (was decomposed 2026-09-07; `wbs.md` now at `workflow-system/product/archive/milestone-15-workflow-supervisor/wbs.md`) (5 WPs: probe M → typed graph L → detect+fire L → context recycle M → exit verify S; **fully serial, no parallel track** — a property of probe-gating, not an oversight). ⚠️ **WP1 IS A GATE: nothing else in the milestone may be built before the probe reports.** Goal, evidence (the measured 10x regression), the ownership boundary with mccc, and the load-bearing "discriminator is ABSENCE, not content" finding are all in `## Milestone 15` below — **read that section first; it is not repeated here.**

**⚠️ M14 IS SPLIT, NOT DONE — do not decompose it as a whole** (`roadmap.md` → Revision 2026-09-06 + a banner on the `### Milestone 14` section). Its **release half SHIPPED as v0.4.0** (2026-09-06: MIT LICENSE + README correctness pass + release note; 2 of 4 deliverables ticked). **Still open and now scheduled AFTER M15:** Settings UI (project-list management + hotkeys, *extending* the `⌘,` panel), Developer-ID signing + notarization, the full two-tier setup docs, repo description/topics. ⚠️ **The order is now M15 → M14's remainder.** ⚠️ **The 2026-08-26 "M14 → M15" reversal is SPENT, not overturned** — its whole argument was that ~40 commits were shipped-but-undelivered ahead of `v0.3.4`; **v0.4.0 delivered them**, so the reason to front-run M14 no longer exists. Do not "restore" the older order by citing that note.
...
## Milestone 15 — the active cycle (inserted 2026-08-14; decomposed 2026-09-07)
