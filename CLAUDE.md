...
- **WP5** ~~`workflow/.session.md` file-watcher~~ — **DROPPED** (wrong file: a manual pause bookmark, not a live workflow-state stream). Replacement idea (watch the live workflow doc hierarchy) deferred to M4+.
...
**Status (2026-06-22):**
- **Milestone 3 (CC lifecycle & state plumbing) — COMPLETE + CLOSED** 2026-06-22 (`/product-finalize`). Critical path WP1→WP2→WP3→WP4→WP6 shipped + live-confirmed (idle→running→awaiting-input observed purely from the hook channel). **WP5 (`.session.md` watcher) was DROPPED** — wrong file (manual pause bookmark, not a live signal); the hook channel alone meets M3's exit criterion. Replacement idea = watch the live workflow doc hierarchy (`roadmap→wbs→wip(s)→backlog`), deferred to M4+ (`SURFACE-2026-06-22-WP5-DROPPED-WATCH-WORKFLOW-DOC-HIERARCHY`). WBS archived at `docs/product/archive/milestone-3-cc-lifecycle-state-plumbing/`. **Next: M4 (multi-workspace UX — filmstrip + center stage)**, the dogfood-replace point — opens via `/product-wbs`.
...
- **Milestone 3 WBS written** 2026-06-22 (commit `5846164`), live at `docs/product/wbs.md`.
- **M3 WP1 (probe) — DONE** 2026-06-22, verdict GO (`docs/product/archive/milestone-3-cc-lifecycle-state-plumbing/wp1-hook-socket-probe-outcome.md`). End-to-end hook→`AF_UNIX`-socket wire confirmed on real macOS; claude-time coexistence proven; listener verdict = blocking `std::os::unix::net::UnixListener` on a dedicated thread (not tokio).
- **M3 WP2 (hook script + `~/.claude/settings.json` registration) — SHIPPED** 2026-06-22 (commit `77d6a6e`). Production Perl hook (`resources/claudesk-hook.pl`) + `hook_install` Rust module (additive/idempotent/reversible merge, atomic write, never-wipe-on-parse-failure) + Tauri `.setup()` install-on-launch wiring + `hook_uninstall` IPC. Live-verified on the real config (additive alongside claude-time/notify-telegram, idempotent re-launch, coexistence). 0 CRITICAL/0 MAJOR review; 4 MINOR backlogged.
  - *Residuals carried:* live `Notification` payload capture deferred to WP6 (SURFACE-2026-06-22-WP1-NOTIFICATION-…); app-data dir is `com.claudesk.app/` not `Claudesk/` — doc sweep at finalize (SURFACE-2026-06-22-APP-DATA-DIR-…).
- **M3 WP3 (`AF_UNIX` socket listener + parse path) — SHIPPED** 2026-06-22 (commit `4355e00`). `hook_socket` module: typed `HookEvent` + pure `parse_line` (snake_case wire keys verbatim, `#[serde(default)]`-tolerant) + `bind_listener` (stale-file removal) + `spawn_listener`/`accept_loop` on a dedicated `std::thread` (blocking `UnixListener`, NOT tokio — WP1's verdict) + `mpsc` delivery into the core (the seam WP4 drains) + `.setup()`/`CloseRequested` lifecycle. `hook_socket::commands::hook_socket_path` is now the single source of truth for the socket path (`hook_install` delegates — writer/reader drift retired). 10 tests; full suite 164/164. 0 CRITICAL/0 MAJOR review; 3 MINOR backlogged (SURFACE-2026-06-22-QUALITY-WP3-MINORS). **Next: WP4** (status broadcaster + `WorkspaceStatusUpdate` DTO + cwd→workspace map + Tauri emit — see `wbs.md`); WP5 (`.session.md` watcher) parallel off WP3.
  - *Residuals carried:* live runtime confirmation (real `claude` hook → live listener end-to-end, incl. the `Notification` payload capture) deferred to **WP6**'s frontend close-the-loop verify-human per the M3 plan — the automated end-to-end test covers the thread+socket+channel path; live binding under the real Tauri app is the WP6 surface.
- **M3 WP4 (status broadcaster + `WorkspaceStatusUpdate` DTO + cwd→workspace map + Tauri emit) — SHIPPED** 2026-06-22 (commit `8bc2d68`). `status_broadcaster` module: pure transform core (`WorkspaceState` enum incl. honest `Unknown` default, `WorkspaceStatusUpdate` snake_case-end-to-end DTO, `WorkspaceRegistry` canonicalized cwd→workspace seam, `event_to_state` + `to_update`) + `commands` runtime wiring (drain thread consuming WP3's held `mpsc::Receiver` on a dedicated `std::thread`, `app.emit("workspace-status", …)`, `SharedRegistry` managed in `.setup()`). Deleted the WP3 module-wide `#![allow(dead_code)]` (its receiver + `prompt`/`timestamp` now live); the WP6-owned residuals (`Unknown`, `register`/`deregister`) carry 3 item-scoped allows naming WP6 as removal owner — no module-wide allow remains. **arch.md convention landed:** IPC DTOs are snake_case end-to-end (no `rename_all`), pinned by a `serde_json::to_value` key-shape test — closes `SURFACE-2026-06-21-IPC-DTO-FIELD-CASE-TESTS-MISS-SERDE-SHAPE`. 16 broadcaster tests (incl. end-to-end socket→transform over real WP3 plumbing); full suite **180/180**. 0 CRITICAL/0 MAJOR review; 3 MINOR backlogged (SURFACE-2026-06-22-QUALITY-WP4-MINORS, cosmetic docstring drift). **Next: WP6** (frontend subscribe + honest indicator + workspace open→register/close→deregister wiring + the live real-`claude`→emit close-the-loop verify-human); **WP5** (`.session.md` watcher) parallel off WP3/WP4.
  - *Residuals carried:* the cwd→workspace **registration** (open→register / close→deregister) + the live real-`claude`→emit close-the-loop are **WP6** deliverables per the WBS — WP4 defines + tests the registry seam + the transform; WP6 wires the registration, the frontend `listen("workspace-status")`, and the live verify-human (the WP1 `Notification` + WP3 live-bind residuals ride along).
