...
**Milestone 7: Menu-bar status item — ambient ALARM + ACTUATOR (shrunk).** A macOS menu-bar (system-tray) item scoped to the one thing the menu bar offers that the shipped M5 PiP does not: **location** (a strip the operator already passively watches all day, present even at zero workspaces, no summon/allocated region). It is **(a) a 2-state ambient alarm** — a template tray icon **lit when ANY workspace is `AwaitingInput`, neutral otherwise** (Running + Idle collapse — running-vs-idle detail is PiP's / the window's job) — and **(b) a native actuator menu** (Show Claudesk / Toggle PiP / Quit). It subscribes to the existing M3 `status_broadcaster` `workspace-status` event (no broadcaster change). It is **NOT** a third status surface.

**SHRUNK at the M7 spec debate (2026-06-29):** the originally-planned popover dashboard (per-workspace list + navigate-on-click + `tauri-plugin-positioner`) was **CUT** — capability-by-capability it was a strict subset of PiP (`On`+`minimal` already gives near-zero-pixel, all-Spaces, always-on aggregate status), so a second dashboard would split the glance + double maintenance, against the "lite / attention is scarce" thesis. Captured as design-prior `new-surface-must-earn-its-place-against-existing-ones`. No `/product-research`; Tauri 2.11.x tray API verified at the arch pass; the one residual API check (`set_icon_and_icon_as_template_atomic` blink-free swap) is an inline WP1 confirmation, not a probe WP.

Work packages (Milestone 7 — live decomposition in `docs/product/wbs.md`). **Linear critical path WP1 → WP2 → WP3** (tiny, tightly-coupled):
- **WP1** Tray icon — ambient 2-state alarm. `src-tauri/src/tray/` (mirrors `pip/`): `aggregate_alarm` pure fold (Attention iff any AwaitingInput; else Neutral), 2 pre-rendered lit/neutral template PNGs swapped via `set_icon_and_icon_as_template_atomic` on each `workspace-status` event + register/deregister. No menu yet, no popover ever.
- **WP2** Native actuator menu — `TrayIconBuilder::menu(...)` Show Claudesk / Toggle PiP / Quit, reusing the 2026-06-24 `app_menu` `on_menu_event`/emit bridge. Actuators are the non-redundant complement (display-only PiP can't act on the app).
- **WP3** Milestone-exit verification — agent GREENs the bridge-observable slice (`aggregate_alarm` unit tests + icon-swap-no-abort on a driven transition); native glyph lit/neutral + menu-appears/items-fire + out-of-focus/cross-Space/full-screen visibility carried as DEFERRED-TO-RELEASE (the M5/M6 pattern).

**Scope decisions baked in:** New module `src-tauri/src/tray/` + a `trayIcon` block in `tauri.conf.json` + 2 template PNGs in `src-tauri/icons/tray/`. **NO new dependency, NO new webview, NO new Vite entry** (all cut with the popover). **Reuses (no new seam):** the M3 `workspace-status` broadcast, the M5 run-on-main-thread rule, the 2026-06-24 `app_menu` managed-handle + `on_menu_event`/emit pattern. Design priors: `new-surface-must-earn-its-place-against-existing-ones` (NEW — governs the shrink); `explicit-selectable-mode-over-inferred-mode` agrees (cut the high-surface popover half); `operator-helpful-friend-misfiring-as-offswitchable-setting` does NOT fire. The doc-hierarchy watcher is re-anchored M7 → **M8** (no popover to host its workflow-position line).

**Status (2026-06-29):**
- **Milestone 7 — JUST PLANNED + SHRUNK** 2026-06-29 (`/session-start` → `/product-arch` → `/product-wbs` → `/product-context`, autopilot; then SHRUNK at the `/feature-spec` debate from a 3-surface dashboard to an ambient alarm + actuator). Arch at `arch.md` §B.2 (designed-for-build — SHRUNK, Revision 2026-06-29); WBS at `docs/product/wbs.md` (3 WPs); spec+plan at `workflow/wip/m7-menu-bar-status-item.md`. **Next: WP1 build (`/feature-build`).** Nothing built yet.
- **Milestone 6 (Friend-requested QoL polish) — COMPLETE + CLOSED + RELEASED as v0.2.2** 2026-06-28 (`/product-finalize`; `/release` v0.2.2 — GitHub release live, Homebrew tap `c532ba7`, installed-`.app` verified). All 12 WPs shipped (WP1/1b status-channel-logging probe → WP2 stuck-`Running` dot fix `bafee80` [longest-prefix/ancestor cwd match] → WP3 split-ratio control → WP4 focus-scoped terminal zoom → WP5 editor auto-wrap → WP6 FileTree heavy-dir re-base → WP7 no-yolo setting → WP9 empty-PiP suppression → WP10 right-panel terminal zoom → WP11 multiple right-panel terminals → WP8 exit verify). The open collection grew exactly as designed. WBS archived at `docs/product/archive/milestone-6-friend-qol/`.
...
- **Roadmap (resequenced dogfood-first 2026-06-22, +friend-QoL/docs-viewer/time-analytics inserts):** M1–M6 complete; execution order onward is **M7 menu-bar** (current) → **M8 docs-viewer** → **M9 time-analytics** → **M10 auto-resume** → **M11 skill-orchestration** → **M12 polish**. See `roadmap.md` revisions.
- **Published:** origin/main in sync (latest `1d01eb9`); releases via the `/release` skill (current: v0.2.2).
